...
async function createFixture() {
  const homeDir = await mkdtemp(
    path.join(os.tmpdir(), "ao-browser-import-home-"),
  );
  const aoDataRoot = await mkdtemp(
    path.join(os.tmpdir(), "ao-browser-import-dest-"),
  );
  const sourceRoot = path.join(homeDir, "Google", "Chrome");
  const profilePath = path.join(sourceRoot, "Default");
  await mkdir(profilePath, { recursive: true });
  await writeFile(
    path.join(profilePath, "Bookmarks"),
    JSON.stringify(sourceBookmarks()),
    "utf8",
  );
  const roots: BrowserImportRoot[] = [
    { browser: "chrome", label: "Google Chrome", path: sourceRoot },
  ];
  const engine = createBrowserImportEngine({
    homeDir,
    platform: "win32",
    aoDataRoot,
    destinationBookmarksPath: path.join(
      aoDataRoot,
      "Partitions",
      "ao-browser",
      "Bookmarks",
...
describe("browser import engine", () => {
  it("normalizes bookmarks, rejects executable URLs, and computes a Chromium checksum", () => {
    ...
    expect(
      normalized?.file.roots.bookmark_bar.children?.[1].children?.[0],
    ).toMatchObject({
      url: "https://ao.example.test/nested",
    });
  });
  ...
  it("imports only the transformed bookmark file into the AO destination", async () => {
    ...
    const persisted = JSON.parse(await readFile(destination, "utf8")) as {
      roots: { bookmark_bar: { children: unknown[] } };
    };
  ...
  it("rejects imports after the persistent destination has been activated", async () => {
      ...
      roots: [
        {
          browser: "chrome",
          label: "Google Chrome",
          path: path.join(homeDir, "Google", "Chrome"),
        },
      ],
    ...
    await expect(
      activeEngine.importSource(activeScan.sources[0].id),
