...
// WorkOS sign-in defaults to a local loopback callback so the desktop app can
// exchange the code itself without relying on the AO website.
// AO_CLOUD_AUTH_REDIRECT can opt back into ao-app://callback or another
// loopback URL when needed.
const REDIRECT_URI =
  process.env.AO_CLOUD_AUTH_REDIRECT?.trim() ||
  "http://127.0.0.1:5174/callback";
const useLoopbackRedirect = /^https?:\/\//i.test(REDIRECT_URI);
const LOOPBACK_FALLBACK_PORTS = [5174, 3000, 3001];
...
export async function beginCloudSignIn(
  ...
  if (useLoopbackRedirect) {
    redirectUri = await startLoopbackCallbackServer(dataDir, notifyRenderers);
  }
...
export async function handleCloudDeepLink(
  ...
  if (url.protocol !== CALLBACK_PROTOCOL) return null;
...
function loopbackRedirectCandidates(): URL[] {
  const primary = new URL(REDIRECT_URI);
  const seen = new Set<string>();
  const candidates: URL[] = [];
  for (const port of [
    Number(primary.port) || 80,
    ...LOOPBACK_FALLBACK_PORTS,
  ]) {
    const candidate = new URL(REDIRECT_URI);
...
async function startLoopbackCallbackServer(
  ...
  closeLoopbackServer();
  for (const redirect of loopbackRedirectCandidates()) {
    const server = createLoopbackCallbackServer(dataDir, redirect, notifyRenderers);
    try {
      await listenOnLoopbackPort(server, redirect);
    } catch (error) {
      server.close();
  ...
  return createServer((req, res) => {
    ...
    void (async () => {
      try {
        ...
        (notifyRenderers ?? notifyRenderersFn)?.(account);
        sendCallbackPage(
          res,
          200,
        ...
        setTimeout(() => {
          if (loopbackServer) closeLoopbackServer();
        }, 250);
      }
    })();
  });
