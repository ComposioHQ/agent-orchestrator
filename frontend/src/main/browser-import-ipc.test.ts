...
function engine() {
  return {
    detect: vi.fn(async () => ({
      sources: [],
      supportedData: ["bookmarks"] as const,
    })),
...
describe("browser import controller", () => {
  it("requires an explicit activation boolean and selects persistence only when requested", async () => {
    ...
    await expect(
      controller.importSource({ sourceId: "source-1" }),
    ).rejects.toThrow(/activation must be explicit/);
  ...
  it("serializes operations and supports dismissing persistent use for future workers", async () => {
    ...
    expect(profileStorage.selectPersistence).toHaveBeenLastCalledWith(
      "ephemeral",
    );
  });
});
...
describe("browser import IPC boundary", () => {
  it("rejects non-shell senders before reaching the controller", async () => {
    const handlers = new Map<
      string,
      (event: { sender: object }, input?: unknown) => unknown
    >();
    const controller = {
      detect: vi.fn(),
      importSource: vi.fn(),
    ...
    await expect(
      Promise.resolve().then(() =>
        handlers.get(BROWSER_IMPORT_DETECT_CHANNEL)!({ sender: {} }),
      ),
  ...
  it("routes only trusted calls to the controller and never adds a path argument", async () => {
    ...
    const result = await handlers.get(BROWSER_IMPORT_IMPORT_CHANNEL)!(
    ...
    await handlers.get(BROWSER_IMPORT_STATUS_CHANNEL)!({
      sender: trustedSender,
    });
    await handlers.get(BROWSER_IMPORT_USE_EPHEMERAL_CHANNEL)!({
      sender: trustedSender,
    });
    ...
    expect(controller.importSource).toHaveBeenCalledWith({
      sourceId: "opaque-id",
      activate: true,
    });
