...
function defaultRoots(platform: NodeJS.Platform, homeDir: string): readonly BrowserImportRoot[] {
	if (platform === "win32") {
		return [
			{ browser: "chrome", label: "Google Chrome", path: path.join(homeDir, "AppData", "Local", "Google", "Chrome", "User Data") },
			{ browser: "edge", label: "Microsoft Edge", path: path.join(homeDir, "AppData", "Local", "Microsoft", "Edge", "User Data") },
			{ browser: "brave", label: "Brave", path: path.join(homeDir, "AppData", "Local", "BraveSoftware", "Brave-Browser", "User Data") },
		];
	}
	if (platform === "darwin") {
		return [
			{ browser: "chrome", label: "Google Chrome", path: path.join(homeDir, "Library", "Application Support", "Google", "Chrome") },
			{ browser: "edge", label: "Microsoft Edge", path: path.join(homeDir, "Library", "Application Support", "Microsoft Edge") },
			{ browser: "brave", label: "Brave", path: path.join(homeDir, "Library", "Application Support", "BraveSoftware", "Brave-Browser") },
		];
	}
	if (platform === "linux") {
		return [
...
function safeTimestamp(value: unknown, now: Date): string {
	return typeof value === "string" && /^\d+$/.test(value) ? value : chromeTimestamp(now);
}
...
function updateChecksum(hash: ReturnType<typeof createHash>, node: NormalizedBookmarkNode): void {
...
export function normalizeBookmarks(
...
): { file: NormalizedBookmarkFile; importedBookmarks: number; skippedBookmarks: number } | null {
	...
	const makeNode = (node: RawBookmarkNode, depth: number): NormalizedBookmarkNode | null => {
	...
	const makeRoot = (source: RawBookmarkNode, id: string, fallback: string, children: unknown[]): NormalizedBookmarkNode => {
		const date = now();
	...
	const normalizedRoots = {
		bookmark_bar: makeRoot(bookmarkBar, "1", "Bookmarks bar", bookmarkBar.children),
...
async function readNormalizedBookmarks(
	bookmarksPath: string,
	options: { randomId: () => string; now: () => Date },
): Promise<{ file: NormalizedBookmarkFile; importedBookmarks: number; skippedBookmarks: number } | null> {
	try {
...
export function createBrowserImportEngine(options: BrowserImportEngineOptions) {
	...
	const detect = async (): Promise<BrowserImportScan> => {
		...
		if (roots.length === 0) {
			if (options.platform !== "win32" && options.platform !== "darwin" && options.platform !== "linux") {
		...
		for (const root of roots) {
			...
			for (const entry of entries) {
				...
				const normalized = await readNormalizedBookmarks(bookmarksPath, { randomId, now });
	...
	const importSource = async (sourceId: string): Promise<BrowserImportResult> => {
		...
		const normalized = await readNormalizedBookmarks(candidate.bookmarksPath, { randomId, now });
		if (!normalized) throw new BrowserImportError("SOURCE_INVALID", "The selected browser bookmarks are not a supported format.");
		if (!isPathInside(aoDataRoot, destinationPath) || path.basename(destinationPath) !== "Bookmarks") {
			throw new BrowserImportError("DESTINATION_NOT_EMPTY", "AO's persistent browser destination is unavailable.");
		}
		try {
			await lstat(destinationPath);
			throw new BrowserImportError("DESTINATION_NOT_EMPTY", "AO's persistent browser destination already contains bookmarks.");
		} catch (error) {
		...
		const destinationParent = await realpath(path.dirname(destinationPath)).catch(() => null);
		if (!destinationParent || !isPathInside(aoDataRoot, destinationParent)) {
			throw new BrowserImportError("DESTINATION_NOT_EMPTY", "AO's persistent browser destination is unavailable.");
		}
		const tmp = path.join(path.dirname(destinationPath), `.Bookmarks-${process.pid}-${Date.now()}.tmp`);
		try {
