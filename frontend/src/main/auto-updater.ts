...
import { accessSync, constants as fsConstants, existsSync, writeFileSync } from "node:fs";
...
async function runSerializedUpdaterOperation(
  ...
  const run = async () => {
    ...
    try {
      ...
      if (stagedJournal.state === "replacing" && !isNativeStagingBusyError(error)) {
...
function wireUpdaterEvents(): void {
  ...
  autoUpdater.on("error", (err) => {
    ...
    if (!hasPendingReplacement() && hasStagedBuild() && isStagedInstallRejection(err)) {
      ...
      persistJournalTransition({ type: "staged-install-rejected" });
      ...
      // Disarming the rejected candidate re-enables auto-download, so the next check can
  ...
  try {
    await runSerializedUpdaterOperation("automatic-check", async () => {
      ...
      try {
        ...
        } else if (settings.enabled || staleStaged) {
          if (
...
// build from Settings. Downloads follow the saved preference, while stale
// channel replacements are always downloaded to supersede the armed build.
// It reports progress via the broadcast status. Updates only work in the packaged,
      ...
      async () => {
        ...
        try {
          ...
          if (!result?.downloadPromise && staleStaged && stagedJournal.state === "replacing") {
            ...
            activeDownloadCancellation = token;
            await autoUpdater.downloadUpdate(token);
          } else if (!result?.downloadPromise && settings.enabled && result?.isUpdateAvailable &&
