...
export function ClaudeCodeAccountLoginTerminalPanel({ activeLogin, pending, onCheckAgain, onClose, onRetry }: {
	activeLogin: ClaudeCodeActiveLogin;
	pending: boolean;
	onCheckAgain: () => void;
	onClose: () => void;
	onRetry: () => void;
	...
	return <div ref={panelRef} className="scroll-my-3 overflow-hidden rounded-md border border-border bg-terminal" data-testid="claude-code-account-login-terminal">
		<div className="flex min-h-10 items-center justify-between gap-3 border-b border-border bg-surface/90 px-3 py-2">
			<div className="min-w-0"><p className="truncate text-xs font-medium text-foreground">{t("settings.claudeCodeAccounts.loginTerminalTitle")}</p><p className="truncate text-[11px] text-muted-foreground" aria-live="polite" role="status">{status}</p></div>
			<button type="button" aria-label={t("settings.claudeCodeAccounts.loginClose")} className="grid size-7 shrink-0 place-items-center rounded text-muted-foreground hover:bg-interactive-hover hover:text-foreground disabled:opacity-50" disabled={pending} onClick={onClose}><X className="size-4" aria-hidden="true" /></button>
		</div>
		...
		{retryable || checkable ? <div className="flex items-center justify-between gap-3 border-t border-border bg-surface/90 px-3 py-2"><p className="min-w-0 text-xs text-muted-foreground" role="alert">{status}</p><div className="flex shrink-0 items-center gap-2">{retryable ? <Button type="button" size="sm" variant="outline" disabled={pending} onClick={onRetry}>{t("settings.claudeCodeAccounts.retry")}</Button> : null}{checkable ? <Button type="button" size="sm" variant="outline" disabled={pending} onClick={onCheckAgain}>{t("settings.claudeCodeAccounts.loginCheckAgain")}</Button> : null}<Button type="button" size="sm" variant="ghost" disabled={pending} onClick={onClose}>{t("settings.claudeCodeAccounts.loginClose")}</Button></div></div> : null}
	</div>;
}
