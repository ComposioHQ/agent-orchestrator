...
function errorMessage(error: unknown, t: TFunction): string {
  ...
  if (/no longer available|could not be read safely/i.test(message))
...
export function BrowserImportSection({
  ...
  const importSelected = async () => {
    ...
    setError(null);
    try {
      const next = await aoBridge.browserImport.import({
    ...
    <SettingsSection title={t("settings.browser")} titleHidden={titleHidden}>
      <div className="flex flex-col gap-3 rounded-md border border-border bg-background p-4">
        <div className="flex items-start gap-3">
          <Globe2
            className="mt-0.5 size-5 shrink-0 text-settings-muted"
            aria-hidden="true"
          />
          <div className="min-w-0 space-y-1">
            ...
            <p className="text-xs leading-relaxed text-settings-muted">
              {t("settings.browser.disclosure")}
            </p>
            <p className="text-xs leading-relaxed text-settings-muted">
              {t("settings.browser.persistentDisclosure")}
            </p>
          </div>
        </div>
        ...
        {status.persistence === "persistent" && (
          <div
            className="flex items-start gap-2 rounded-md border border-success/30 bg-success/10 p-3 text-xs text-success"
            role="status"
          >
        ...
        {status.summary && (
          <p className="text-xs text-settings-muted">
            {t("settings.browser.success", {
              count: status.summary.importedBookmarks,
            })}
          </p>
        )}
        ...
        {flowState === "dismissed" && (
          <div className="flex flex-wrap items-center gap-2">
            <p className="text-xs text-settings-muted" role="status">
              {t("settings.browser.dismissed")}
            </p>
        ...
        {flowState === "scanning" && (
          <p
            className="flex items-center gap-2 text-xs text-settings-muted"
            role="status"
          >
        ...
        {(flowState === "ready" || flowState === "importing") && (
          <div className="flex flex-col gap-3">
            {sources.length === 0 ? (
              <p className="text-xs text-settings-muted" role="status">
                {t("settings.browser.noSources")}
              </p>
            ) : (
              <fieldset className="flex flex-col gap-2">
                <legend className="text-xs font-medium text-foreground">
                  {t("settings.browser.sources")}
                </legend>
                {sources.map((source) => (
                  <label
                    key={source.id}
            ...
            <div className="flex items-start gap-2 rounded-md border border-border bg-muted/20 p-3">
              <ShieldCheck
                className="mt-0.5 size-4 shrink-0 text-settings-muted"
                aria-hidden="true"
              />
              <label className="flex cursor-pointer items-start gap-2 text-xs leading-relaxed text-settings-muted">
                <input
                  ...
                  onChange={(event) =>
                    setActivatePersistent(event.target.checked)
                  }
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                {flowState === "importing"
                  ? t("settings.browser.importing")
                  : t("settings.browser.importAndUse")}
              </Button>
              <Button
                type="button"
                variant="ghost"
        ...
        {flowState === "success" && result && (
          <div className="flex flex-col gap-2" role="status" aria-live="polite">
            ...
            {result.skippedBookmarks > 0 && (
              <p className="text-xs text-settings-muted">
                {t("settings.browser.successSkipped", {
                  count: result.importedBookmarks,
