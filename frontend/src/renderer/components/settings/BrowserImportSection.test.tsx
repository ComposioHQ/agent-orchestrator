...
beforeEach(() => {
  ...
  useEphemeral.mockResolvedValue({
    persistence: "ephemeral",
    destinationActive: false,
    summary: null,
  });
...
describe("BrowserImportSection", () => {
  it("discloses the bookmark-only scope and keeps import disabled until explicit opt-in", async () => {
    ...
    expect(
      await screen.findByText(/AO reads only bookmark files/i),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/does not import passwords, extensions, cookies/i),
    ).toBeInTheDocument();
    await userEvent.click(
      screen.getByRole("button", { name: "Find supported browser profiles" }),
    );
  ...
  it("scans, requires the persistent opt-in, and imports the selected source", async () => {
    ...
    await user.click(
      await screen.findByRole("button", {
        name: "Find supported browser profiles",
      }),
    );
    const radio = await screen.findByRole("radio", {
      name: /Google Chrome.*Default profile.*4 bookmarks/i,
    });
    ...
    await user.click(
      screen.getByRole("checkbox", { name: /Use the imported bookmarks/i }),
    );
    ...
    expect(
      (await screen.findAllByText(/Imported 4 bookmarks/)).length,
    ).toBeGreaterThan(0);
    expect(
      screen.getByText(/unsupported entries were skipped/),
    ).toBeInTheDocument();
  });
  ...
  it("supports dismissal and retry after a detection failure", async () => {
    ...
    expect(
      await screen.findByRole("radio", { name: /Google Chrome/ }),
    ).toBeInTheDocument();
    await user.click(
      screen.getByRole("button", { name: "Keep temporary browser" }),
    );
    expect(await screen.findByRole("status")).toHaveTextContent(
      "No browser data was imported",
    );
  });
  ...
  it("shows persistent status and allows switching future workers back to temporary storage", async () => {
    ...
    expect(
      await screen.findByText(/persistent browser is enabled/i),
    ).toBeInTheDocument();
    await user.click(
      screen.getByRole("button", {
        name: "Use temporary browser for new workers",
      }),
    );
