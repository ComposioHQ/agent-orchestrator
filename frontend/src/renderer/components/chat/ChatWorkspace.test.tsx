...
function idleSnapshot(
  snapshot: ConversationSnapshot = chatFixture,
): ConversationSnapshot {
  return {
    ...snapshot,
...
describe("HumanMessage attachments", () => {
  ...
  it.each([
    ...
    "renders an AO-generated %s image reference as an image",
    (_source, header, name) => {
  ...
  it("preserves authored trailing whitespace before generated references", () => {
    ...
    expect(
      container.querySelector(".cursor-chat-human-message > p")?.textContent,
    ).toBe(authoredBody);
  });
  ...
  it("shows non-image attachments as file labels instead of internal prompt text", () => {
    ...
    expect(
      screen.getByRole("img", { name: "attachment-ab12.png" }),
    ).toBeInTheDocument();
    expect(screen.getByText("attachment-cd34.pdf")).toBeInTheDocument();
    expect(
      screen.getByRole("list", { name: "Attached files" }),
    ).toBeInTheDocument();
    expect(
      screen.queryByText(/Attached files \(read these files/),
    ).not.toBeInTheDocument();
  });
...
describe("Chat message timestamps", () => {
  ...
  it("labels yesterday and older messages with calendar dates", () => {
    ...
    expect(
      screen.getByLabelText(/^Sent Yesterday · \d{2}:\d{2}$/),
    ).toBeInTheDocument();
    expect(
      screen.getByLabelText(/^Sent [A-Z][a-z]{2} \d{1,2}, \d{4}$/),
    ).toBeInTheDocument();
  });
});
...
describe("ChatWorkspace timeline", () => {
  it("makes composer and history controls inert while a durable agent switch owns input", () => {
    ...
    expect(screen.getByTestId("chat-conversation-panel")).toHaveAttribute(
      "inert",
    );
  });
  ...
  it("uses the shared session topbar chrome for workers and orchestrators", () => {
    const view = render(
      <ChatWorkspace snapshot={chatFixture} sessionRole="worker" />,
    );
    ...
    expect(
      screen.getByRole("tab", { name: chatFixture.title }),
    ).toBeInTheDocument();

    view.rerender(
      <ChatWorkspace snapshot={chatFixture} sessionRole="orchestrator" />,
    );
  ...
  it("leaves new-terminal and display controls out of the chat strip, like the terminal session", () => {
    ...
    expect(terminalRegion).toContainElement(
      screen.getByRole("tablist", { name: "Chat tabs" }),
    );
  ...
  it("shows live working state inline with the current turn while the composer owns the stop action", async () => {
    ...
    expect(screen.getByRole("log", { name: "Conversation" })).toContainElement(
      status,
    );
    ...
    expect(
      screen.getByLabelText("Message the agent").closest("form"),
    ).toContainElement(stop);
  ...
  it("moves a pending approval into the chat composer", async () => {
    ...
    expect(
      screen.getByText("Do you want to run this command?"),
    ).toBeInTheDocument();
    expect(
    ...
    ).not.toContainElement(approval);
    expect(
      screen.queryByLabelText("Message the agent"),
    ).not.toBeInTheDocument();
    expect(within(approval).queryByText("Terminal")).not.toBeInTheDocument();
    expect(
      within(approval).getByRole("button", { name: /Deny/ }),
    ).toHaveTextContent("DenyEsc");
    expect(
      within(approval).getByRole("button", { name: /Allow once/ }),
    ).toBeInTheDocument();
    expect(
      within(approval).getByRole("button", { name: "More approval options" }),
    ).toBeInTheDocument();
    expect(
      within(approval).queryByRole("button", { name: "Allow Once" }),
    ).not.toBeInTheDocument();
    expect(
      within(approval).queryByRole("button", { name: "Always Allow" }),
    ).not.toBeInTheDocument();
    expect(
      within(approval).queryByRole("button", { name: "Deny" }),
    ).not.toBeInTheDocument();
    await user.click(
      within(approval).getByRole("button", { name: "More approval options" }),
    );
    ...
    await user.click(
      screen.getByRole("menuitem", { name: "Always allow this command" }),
    );
  ...
  it("uses ACP edit metadata and semantic kinds instead of guessing opaque labels", () => {
    ...
    expect(
      screen.getByText("Do you want to allow these file changes?"),
    ).toBeInTheDocument();
    expect(
      screen.queryByText("Do you want to run this command?"),
    ).not.toBeInTheDocument();
    const approval = screen.getByRole("group", {
  ...
  it.each([
    ...
    "keeps a resolved %s approval compact and explicit",
    (decision, label) => {
      const snapshot = structuredClone(chatFixture);
      snapshot.items = [
        {
      ...
      expect(
        screen.getByText("Run gh pr create --base main --head ao/example"),
      ).toBeInTheDocument();
      expect(
        screen.queryByText(/req approval-resolved-1/),
      ).not.toBeInTheDocument();
      expect(
        screen.queryByText(
  ...
  it("lets readers select conversation text", () => {
    ...
    expect(screen.getByRole("log", { name: "Conversation" })).toHaveClass(
      "select-text",
    );
  });
  ...
  it("does not report the intentional controller gap during an interface handoff as a crash", () => {
    ...
    expect(
      screen.queryByText("The agent controller stopped"),
    ).not.toBeInTheDocument();
    expect(
  ...
  it("announces thread and tool-server failures", () => {
    const { rerender } = render(
      <ChatWorkspace snapshot={chatFixtureThreadError} />,
    );
    expect(screen.getByRole("alert")).toHaveTextContent(
      "thread hit an internal error",
    );

    rerender(<ChatWorkspace snapshot={chatFixtureMcpFailed} />);
    expect(screen.getByRole("status")).toHaveTextContent(
      /tool servers? did not start/,
    );
  });
  ...
  it("provides an interactive conversation minimap", () => {
    ...
    expect(
      Number.parseFloat(markers[1]!.style.top) -
  ...
  it("hides conversation minimap markers while the inspector is open", () => {
    ...
    expect(
      scrollbar.querySelectorAll("[data-chat-scroll-marker]"),
    ).toHaveLength(0);
  });
  ...
  it("previews the request and response for a hovered conversation marker", () => {
    ...
    expect(preview).toHaveTextContent(
      "Wire the snapshot endpoint into the handler (round 1)",
    );
    expect(preview).toHaveTextContent(
      "Done. conversation now returns the durable snapshot",
    );
  ...
  it("keeps a turn as one block, positioned by its first item", () => {
    ...
    const relayCard = screen.getByText(
      /Checks failed on the base branch/,
    ).parentElement;
  ...
  it("does not yank a reader who scrolled up when the next poll arrives", async () => {
    ...
    rerender(
      <ChatWorkspace snapshot={{ ...poll(snapshot), latestSequence: 999 }} />,
    );
    expect(log.scrollTop).toBe(1200);
  });
  ...
  it("keeps a trailing spacer so a dropped prompt can sit near the top", async () => {
    ...
    const anchors = log.querySelectorAll<HTMLElement>(
    ...
    Object.defineProperty(log, "scrollHeight", {
      configurable: true,
      get: () =>
        contentHeight + (Number.parseFloat(spacer.style.height || "0") || 0),
    });
...
describe("automation reports", () => {
  it("collapses a long report until the reader asks to expand it", async () => {
    const user = userEvent.setup();
    const source = chatFixture.items.find(
      (item) => item.id === "m-4",
    ...
    expect(
      screen.getByRole("button", { name: "Show full report" }),
  ...
  it("keeps a short automation alert fully visible", () => {
    const message = chatFixture.items.find(
      (item) => item.id === "m-4",
    ) as ConversationMessage;
    render(<OriginMessage message={message} />);
    expect(
      screen.getByText(/Checks failed on the base branch/),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: "Show full report" }),
    ).not.toBeInTheDocument();
  });
});
...
describe("ChatWorkspace message actions", () => {
  it("hides first-message editing until the conversation controller is ready", () => {
    ...
    expect(
      screen.queryByRole("button", { name: "Edit user message" }),
    ).not.toBeInTheDocument();
  });
  ...
  it("offers exact first-message edits even when the provider cannot branch or replay history", async () => {
    ...
    expect(
      screen.queryByText(/Reconstructed context:/),
    ).not.toBeInTheDocument();
  ...
  it("offers reconstructed historical edits only when text replay and embedded context are both negotiated", async () => {
    ...
    expect(
      screen.getAllByRole("button", { name: "Edit user message" }),
    ).toHaveLength(1);
    ...
    await user.click(
      screen.getAllByRole("button", { name: "Edit user message" })[1]!,
    );
  ...
  it("treats the earliest loaded prompt as historical while older pages remain", async () => {
    ...
    await user.click(
      screen.getAllByRole("button", { name: "Edit user message" })[0]!,
    );
    expect(screen.getByText(/Reconstructed context:/)).toBeVisible();
  });
  ...
  it("copies a human message as the exact text the user sent", async () => {
    ...
    await user.click(
      screen.getAllByRole("button", { name: "Copy user message" })[0]!,
    );
  ...
  it("retains the inline draft when branch creation fails", async () => {
    ...
    await waitFor(() =>
      expect(onEditMessage).toHaveBeenCalledWith("turn-2", "keep this draft"),
    );
  ...
  it("navigates prompt branches and explains that files are unchanged", async () => {
    ...
    render(
      <ChatWorkspace snapshot={snapshot} onActivateBranch={onActivateBranch} />,
    );
...
describe("ChatWorkspace reviewer tabs", () => {
  ...
  it("keeps the chat draft, attachments, edit, and scroll state mounted while Reviewer is selected", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByLabelText("Remove review.png")).toBeInTheDocument(),
    );
    ...
    view.rerender(
      <ChatWorkspace {...common} reviewerTarget={reviewerTarget} />,
    );
    ...
    expect(screen.getByRole("combobox", { name: "Message the agent" })).toBe(
      composer,
    );
  ...
  it("keeps the selected reviewer pane through temporary terminal unavailability", () => {
    ...
    const view = render(
      <ChatWorkspace {...common} reviewerTerminal={reviewerTerminal} />,
    );
    ...
    expect(screen.getByTestId("chat-reviewer-terminal")).toBeInTheDocument();
    expect(
      screen.queryByRole("tab", { name: "Reviewer" }),
    ).not.toBeInTheDocument();
  });
  ...
  it("gives the reviewer terminal working zoom and fullscreen controls", async () => {
    ...
    expect(terminalPaneState.props?.onChangeFontSize).toEqual(
      expect.any(Function),
    );
    expect(terminalPaneState.props?.onToggleFullscreen).toEqual(
      expect.any(Function),
    );
  ...
  it("supports arrow, Home/End, and desktop previous/next tab shortcuts", () => {
    ...
    expect(
      screen.getByRole("tab", { name: chatFixture.sessionId }),
...
describe("ChatWorkspace shell tabs", () => {
  ...
  it("renders a selected shell as the tab body and hides, not unmounts, the conversation", () => {
    ...
    expect(
      screen.getByRole("tab", { name: "chat worktree shell" }),
  ...
  it("cycles chat → reviewer → shells → chat on the desktop next-tab shortcut", () => {
    ...
    view.rerender(
      <ChatWorkspace {...common} shellTarget={shellTarget("shell-2")} />,
    );
    act(() => [...nextTabListeners][0]?.());
    expect(onSelectChat).toHaveBeenCalledOnce();
  });
  ...
  it("cycles shell → reviewer → chat on the desktop previous-tab shortcut", () => {
    ...
    const view = render(
      <ChatWorkspace {...common} shellTarget={shellTarget("shell-1")} />,
    );
