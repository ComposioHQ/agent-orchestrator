...
- **`context.ts`** — normalizes the visit into `source` / `campaign` /
  `user_type` / `device`. Pure and unit-tested.
- **`events.ts`** — the Product-Hunt-specific events: `ph_referral_visit`,
  `ph_badge_click`, `ph_upvote_cta_click`, `ph_comment_cta_click`, plus
  `return_visit`.
- **`LaunchAnalytics`** (`app/components/LaunchAnalytics`) — mounted once in the
  layout. It registers `source`/`campaign`/`device` as PostHog
  **super-properties**, so from then on **every** event, including autocaptured
  pageviews and the existing `download_clicked` / `waitlist_signup` /
  `section_viewed`, can be broken down by launch source **without editing any
  of those call sites**. It also fires `ph_referral_visit` (once per tab) and
  `return_visit`.
- **`ProductHuntBadge`** (`app/components/ProductHuntBadge`) — a drop-in badge
  linking to our Product Hunt page that fires `ph_badge_click`.
...
| YouTube | `youtube` | `https://aoagents.dev/?utm_source=youtube&utm_medium=social&utm_campaign=launch_day` |
