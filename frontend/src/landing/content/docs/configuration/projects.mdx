...
Built-in reviewer harnesses are `aider`, `agy`, `amp`, `auggie`, `autohand`, `claude-code`, `cline`, `codex`, `continue`, `copilot`, `crush`, `cursor`, `devin`, `droid`, `goose`, `grok`, `kilocode`, `kimchi`, `kiro`, `kimi`, `opencode`, `pi`, `qwen`, and `vibe`. Kilo Code requires the
...
Agy, Devin, Droid, Kimchi, Kimi, Qwen, and Vibe reviewer support is experimental and host-trusted. Grok, Crush, Auggie, Cline, and Autohand are experimental user-approved reviewers: AO strengthens their reviewer system prompt and leaves native permission prompts enabled instead of passing broad unattended flags. Their readonly, plan, or
