...
`defaultBranch: "auto"` resolves each repository's Git default independently. AO prefers `origin/HEAD`, then the checked-out branch, and uses `main` only as a last resort. Set an explicit branch when a project should remain pinned to that ref.
