...
AO never treats the currently checked-out branch, `main`, or `master` as an inferred default. If a repository has no authoritative remote default (or has ambiguous remotes), spawn returns `DEFAULT_BRANCH_UNRESOLVED`; set `defaultBranch` explicitly or configure `checkout.defaultRemote`. Repositories initialized by AO are the exception because AO records the `main` choice when it creates them.
