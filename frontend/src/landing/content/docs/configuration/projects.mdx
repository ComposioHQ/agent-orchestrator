...
Codex Chat also supports the preventive `read-only` permission mode. Configure it in `agentConfig` or pass `ao spawn --permission read-only` for one session. It runs without approval prompts while the provider sandbox blocks filesystem writes. AO rejects this mode for other Chat drivers and Terminal UI sessions instead of silently granting broader access.
