...
Automatic resolution uses the selected remote's live symbolic `HEAD`, with its cached `HEAD` as an offline fallback. It does not use the branch currently checked out in the source repository. If Git does not expose a trustworthy default, AO asks for explicit `defaultBranch` configuration instead of guessing.
