...
describe("shared UI locale schema", () => {
	...
	it("normalizes persisted settings through the shared locale validator", () => {
		...
		expect(coerceUiSettings({ locale: "ja" })).toEqual({ locale: "ja", soundNotificationsEnabled: true, cloudBetaEnabled: false });
		expect(coerceUiSettings({ locale: "pt-BR" })).toEqual({ locale: "pt-BR", soundNotificationsEnabled: true, cloudBetaEnabled: false });
	...
	it("defaults soundNotificationsEnabled to true and accepts a persisted boolean", () => {
		expect(DEFAULT_UI_SETTINGS).toEqual({ locale: "en", soundNotificationsEnabled: true, cloudBetaEnabled: false });
	...
	it("coerces a non-boolean or missing soundNotificationsEnabled to the default (true)", () => {
		expect(coerceUiSettings({ locale: "en" })).toEqual({ locale: "en", soundNotificationsEnabled: true, cloudBetaEnabled: false });
		expect(coerceUiSettings({ locale: "en", soundNotificationsEnabled: "false" })).toEqual({
			locale: "en",
			soundNotificationsEnabled: true,
			cloudBetaEnabled: false,
