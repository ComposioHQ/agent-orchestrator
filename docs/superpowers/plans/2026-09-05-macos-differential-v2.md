...
3. Implement an explicit v2 resolver with bounded authenticated metadata/map reads, exact candidate/baseline validation and no discovery through legacy blockmap URLs.
...
Step 6: 251 focused tests pass on actual Node 24.20.0; project and E2E typechecks
and unsigned packaging pass. CI and independent review follow on the existing PR.
Packaged v2 Electron net/native acceptance, production keys and conductor rollout
authorization remain unimplemented acceptance gates; the feature stays disabled.
