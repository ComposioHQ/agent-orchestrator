...
function mockTmuxWithProcess(processName = "continue", tty = "/dev/ttys001", pid = 12345) {
...
describe("plugin manifest & exports", () => {
  it("has correct manifest metadata", () => {
    expect(manifest).toEqual({
      name: "continue",
      slot: "agent",
      description: "Agent plugin: Continue",
      version: "0.1.0",
    });
  });
  ...
  it("create() returns an agent with correct name and processName", () => {
    ...
    expect(agent.processName).toBe("continue");
  });
...
describe("getLaunchCommand", () => {
  ...
  it("generates base command 'continue' with no extra flags", () => {
    const cmd = agent.getLaunchCommand(makeLaunchConfig());
    expect(cmd).toBe("continue");
  });

  it("includes --auto when permissions=skip", () => {
    const cmd = agent.getLaunchCommand(makeLaunchConfig({ permissions: "skip" }));
    expect(cmd).toContain("--auto");
  });

  it("does not include --auto when permissions is not skip", () => {
    const cmd = agent.getLaunchCommand(makeLaunchConfig());
  ...
  it("shell-escapes prompt argument with -p flag", () => {
  ...
  it("combines all options", () => {
    ...
    expect(cmd).toBe("continue --auto --model 'opus' -p 'Hello'");
  });
...
describe("isProcessRunning", () => {
  ...
  it("returns true when continue is found on tmux pane TTY", async () => {
    mockTmuxWithProcess("continue");
    expect(await agent.isProcessRunning(makeTmuxHandle())).toBe(true);
  });

  it("returns false when no continue process on tmux pane TTY", async () => {
  ...
  it("finds continue on any pane in multi-pane session", async () => {
    mockExecFileAsync.mockImplementation((cmd: string, args: string[]) => {
      if (cmd === "tmux" && args[0] === "list-panes") {
  ...
  it("does not match similar process names like continue-extra", async () => {
    mockExecFileAsync.mockImplementation((cmd: string, args: string[]) => {
      if (cmd === "tmux" && args[0] === "list-panes") {
        return Promise.resolve({ stdout: "/dev/ttys001\n", stderr: "" });
      }
      if (cmd === "ps") {
        return Promise.resolve({
          stdout: "  PID TT ARGS\n  100 ttys001  /usr/bin/continue-extra\n",
          stderr: "",
        });
      }
      return Promise.reject(new Error("unexpected"));
...
describe("getSessionInfo", () => {
  ...
  it("returns null (Continue has no JSONL introspection yet)", async () => {
...
describe("getActivityState", () => {
  ...
  it("returns null (Continue has no session introspection yet)", async () => {
