...
describe("getLaunchCommand", () => {
  ...
  it("generates base command 'gemini' with no extra flags", () => {
  ...
  it("includes --system-instruction with systemPrompt", () => {
  ...
  it("combines all options", () => {
    ...
    expect(cmd).toBe("gemini --yolo --model 'opus' -p 'Hello'");
  });
  ...
  it("omits optional flags when not provided", () => {
    ...
    expect(cmd).not.toContain("--system-instruction");
  });
});
...
describe("isProcessRunning", () => {
  ...
  it("returns false when no gemini process on tmux pane TTY", async () => {
  ...
  it("does not match similar process names like gemini-pro", async () => {
    mockExecFileAsync.mockImplementation((cmd: string, args: string[]) => {
      if (cmd === "tmux" && args[0] === "list-panes") {
        return Promise.resolve({ stdout: "/dev/ttys001\n", stderr: "" });
      }
      if (cmd === "ps") {
        return Promise.resolve({
          stdout: "  PID TT ARGS\n  100 ttys001  /usr/bin/gemini-pro\n",
          stderr: "",
        });
      }
      return Promise.reject(new Error("unexpected"));
...
describe("getSessionInfo", () => {
  ...
  it("returns null (Gemini CLI has no JSONL introspection yet)", async () => {
...
describe("getActivityState", () => {
  ...
  it("returns null (Gemini CLI has no session introspection yet)", async () => {
