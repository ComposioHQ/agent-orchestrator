...
/**
 * Check if a process named "gemini" is running in the given runtime handle's context.
...
async function findGeminiProcess(handle: RuntimeHandle): Promise<number | null> {
  try {
    // For tmux runtime, get the pane TTY and find gemini on it
    if (handle.runtimeName === "tmux" && handle.id) {
      ...
      // Match "gemini" as a word boundary
...
/** Classify Gemini CLI's activity state from terminal output (pure, sync). */
function classifyTerminalOutput(terminalOutput: string): ActivityState {
  ...
  // The ❯ is Gemini CLI's prompt character.
...
async function setupHookInWorkspace(workspacePath: string, hookCommand: string): Promise<void> {
  ...
  const hookScriptPath = join(geminiDir, "metadata-updater.sh");
...
function createGeminiAgent(): Agent {
  return {
    ...
    async isProcessRunning(handle: RuntimeHandle): Promise<boolean> {
      const pid = await findGeminiProcess(handle);
      return pid !== null;
    },
    ...
    async setupWorkspaceHooks(workspacePath: string, _config: WorkspaceHooksConfig): Promise<void> {
      ...
      const hookScriptPath = join(workspacePath, ".gemini", "metadata-updater.sh");
    ...
    async postLaunchSetup(session: Session): Promise<void> {
      ...
      // Use absolute path for hook command (specific to this workspace)
      const hookScriptPath = join(session.workspacePath, ".gemini", "metadata-updater.sh");
