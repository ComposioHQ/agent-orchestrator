...
function mockTmuxWithProcess(processName = "kiro", tty = "/dev/ttys001", pid = 12345) {
...
describe("plugin manifest & exports", () => {
  ...
  it("create() returns an agent with correct name and processName", () => {
    ...
    expect(agent.processName).toBe("kiro");
  });
...
describe("getLaunchCommand", () => {
  ...
  it("shell-escapes prompt with --no-interactive flag", () => {
  ...
  it("combines permissions and prompt", () => {
    ...
    expect(cmd).toBe("kiro --trust-all-tools --no-interactive 'Hello'");
  });
...
describe("isProcessRunning", () => {
  ...
  it("returns true when kiro is found on tmux pane TTY", async () => {
    mockTmuxWithProcess("kiro");
    expect(await agent.isProcessRunning(makeTmuxHandle())).toBe(true);
  });

  it("returns false when no kiro process on tmux pane TTY", async () => {
  ...
  it("finds kiro on any pane in multi-pane session", async () => {
    mockExecFileAsync.mockImplementation((cmd: string, args: string[]) => {
      if (cmd === "tmux" && args[0] === "list-panes") {
      ...
      if (cmd === "ps") {
        return Promise.resolve({
          stdout: "  PID TT ARGS\n  100 ttys001  bash\n  200 ttys002  kiro --no-interactive test\n",
          stderr: "",
        });
      }
      return Promise.reject(new Error("unexpected"));
  ...
  it("does not match similar process names like kiro-extra", async () => {
    mockExecFileAsync.mockImplementation((cmd: string, args: string[]) => {
      if (cmd === "tmux" && args[0] === "list-panes") {
        return Promise.resolve({ stdout: "/dev/ttys001\n", stderr: "" });
      }
      if (cmd === "ps") {
        return Promise.resolve({
          stdout: "  PID TT ARGS\n  100 ttys001  /usr/bin/kiro-extra\n",
          stderr: "",
        });
      }
      return Promise.reject(new Error("unexpected"));
...
describe("getSessionInfo", () => {
  ...
  it("returns null (Kiro has no JSONL introspection yet)", async () => {
...
describe("getActivityState", () => {
  ...
  it("returns null (Kiro has no session introspection yet)", async () => {
