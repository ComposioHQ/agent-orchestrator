...
function mockTmuxWithProcess(processName = "cursor", tty = "/dev/ttys001", pid = 12345) {
...
describe("plugin manifest & exports", () => {
  it("has correct manifest metadata", () => {
    expect(manifest).toEqual({
      name: "cursor",
      slot: "agent",
      description: "Agent plugin: Cursor",
      version: "0.1.0",
    });
  });
...
describe("getLaunchCommand", () => {
  ...
  it("generates base command 'cursor' with no extra flags", () => {
    const cmd = agent.getLaunchCommand(makeLaunchConfig());
    expect(cmd).toBe("cursor");
  });

  it("includes --trust when permissions=skip", () => {
    const cmd = agent.getLaunchCommand(makeLaunchConfig({ permissions: "skip" }));
    expect(cmd).toContain("--trust");
  });

  it("does not include --trust when permissions is not skip", () => {
    const cmd = agent.getLaunchCommand(makeLaunchConfig());
  ...
  it("combines all options", () => {
    ...
    expect(cmd).toBe("cursor --trust --model 'opus' -p 'Hello'");
  });
...
describe("isProcessRunning", () => {
  ...
  it("returns true when cursor is found on tmux pane TTY", async () => {
    mockTmuxWithProcess("cursor");
    expect(await agent.isProcessRunning(makeTmuxHandle())).toBe(true);
  });

  it("returns false when no cursor process on tmux pane TTY", async () => {
  ...
  it("does not match similar process names like cursor-extra", async () => {
    mockExecFileAsync.mockImplementation((cmd: string, args: string[]) => {
      if (cmd === "tmux" && args[0] === "list-panes") {
        return Promise.resolve({ stdout: "/dev/ttys001\n", stderr: "" });
      }
      if (cmd === "ps") {
        return Promise.resolve({
          stdout: "  PID TT ARGS\n  100 ttys001  /usr/bin/cursor-extra\n",
          stderr: "",
        });
      }
      return Promise.reject(new Error("unexpected"));
...
describe("getSessionInfo", () => {
  ...
  it("returns null (Cursor has no JSONL introspection yet)", async () => {
...
describe("getActivityState", () => {
  ...
  it("returns null (Cursor has no session introspection yet)", async () => {
