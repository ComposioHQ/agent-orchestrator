...
function mockTmuxWithProcess(processName = "q", tty = "/dev/ttys001", pid = 12345) {
...
describe("plugin manifest & exports", () => {
  it("has correct manifest metadata", () => {
    expect(manifest).toEqual({
      name: "amazon-q",
      slot: "agent",
      description: "Agent plugin: Amazon Q Developer CLI",
      version: "0.1.0",
    });
  });
  ...
  it("create() returns an agent with correct name and processName", () => {
    const agent = create();
    expect(agent.name).toBe("amazon-q");
    expect(agent.processName).toBe("q");
  });
...
describe("getLaunchCommand", () => {
  ...
  it("generates base command 'q chat --no-interactive'", () => {
    const cmd = agent.getLaunchCommand(makeLaunchConfig());
    expect(cmd).toBe("q chat --no-interactive");
  });
  ...
  it("does not include --trust-all-tools when permissions is not skip", () => {
    const cmd = agent.getLaunchCommand(makeLaunchConfig());
    expect(cmd).not.toContain("--trust-all-tools");
  });

  it("shell-escapes prompt argument with --no-interactive", () => {
  ...
  it("combines permissions and prompt", () => {
    ...
    expect(cmd).toBe("q chat --trust-all-tools --no-interactive 'Hello'");
  });
...
describe("isProcessRunning", () => {
  ...
  it("returns true when q is found on tmux pane TTY", async () => {
    mockTmuxWithProcess("q");
    expect(await agent.isProcessRunning(makeTmuxHandle())).toBe(true);
  });

  it("returns false when no q process on tmux pane TTY", async () => {
  ...
  it("finds q on any pane in multi-pane session", async () => {
    mockExecFileAsync.mockImplementation((cmd: string, args: string[]) => {
      if (cmd === "tmux" && args[0] === "list-panes") {
  ...
  it("does not match similar process names like q-extra", async () => {
    mockExecFileAsync.mockImplementation((cmd: string, args: string[]) => {
      if (cmd === "tmux" && args[0] === "list-panes") {
        return Promise.resolve({ stdout: "/dev/ttys001\n", stderr: "" });
      }
...
describe("getSessionInfo", () => {
  ...
  it("returns null (Amazon Q CLI has no JSONL introspection yet)", async () => {
...
describe("getActivityState", () => {
  ...
  it("returns null (Amazon Q CLI has no session introspection yet)", async () => {
