...
describe("getLaunchCommand", () => {
  ...
  it("generates base command with no additional flags", () => {
    expect(agent.getLaunchCommand(makeLaunchConfig())).toBe("amp");
  });

  it("ignores model config (Amp has no --model flag)", () => {
    const cmd = agent.getLaunchCommand(makeLaunchConfig({ model: "smart" }));
    expect(cmd).toBe("amp");
  });

  it("ignores prompt (delivered post-launch, not inline)", () => {
    const cmd = agent.getLaunchCommand(makeLaunchConfig({ prompt: "Fix the tests" }));
    expect(cmd).toBe("amp");
  });

  it("ignores all optional configs", () => {
    const cmd = agent.getLaunchCommand(
      makeLaunchConfig({ model: "smart", prompt: "Go", permissions: "permissionless" }),
    );
    expect(cmd).toBe("amp");
  });
});
...
describe("getActivityState", () => {
  ...
  it("returns exited when process is not running", async () => {
    ...
    const session = makeSession({ runtimeHandle: handle });
    mockTmuxWithProcess("amp", false);
  ...
  it("returns ready when process is running", async () => {
    const handle = makeTmuxHandle();
    const session = makeSession({ runtimeHandle: handle });
