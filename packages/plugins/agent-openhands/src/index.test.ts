...
function mockTmuxWithProcess(processName = "openhands", tty = "/dev/ttys001", pid = 12345) {
...
describe("plugin manifest & exports", () => {
  it("has correct manifest metadata", () => {
    expect(manifest).toEqual({
      name: "openhands",
      slot: "agent",
      description: "Agent plugin: OpenHands",
      version: "0.1.0",
    });
  });
  ...
  it("create() returns an agent with correct name and processName", () => {
    ...
    expect(agent.processName).toBe("openhands");
  });
...
describe("getLaunchCommand", () => {
  ...
  it("escapes single quotes in prompt using POSIX method", () => {
    ...
    expect(cmd).toContain("-t 'it'\\''s a test'");
  });

  it("does not include permissions flag (headless mode is inherently autonomous)", () => {
  ...
  it("generates command with prompt", () => {
    ...
    expect(cmd).toBe("openhands --headless -t 'Hello'");
  });

  it("omits --headless and -t flags when no prompt provided", () => {
    const cmd = agent.getLaunchCommand(makeLaunchConfig());
...
describe("isProcessRunning", () => {
  ...
  it("returns true when openhands is found on tmux pane TTY", async () => {
    mockTmuxWithProcess("openhands");
    expect(await agent.isProcessRunning(makeTmuxHandle())).toBe(true);
  });

  it("returns false when no openhands process on tmux pane TTY", async () => {
  ...
  it("finds openhands on any pane in multi-pane session", async () => {
    mockExecFileAsync.mockImplementation((cmd: string, args: string[]) => {
      if (cmd === "tmux" && args[0] === "list-panes") {
      ...
      if (cmd === "ps") {
        return Promise.resolve({
          stdout: "  PID TT ARGS\n  100 ttys001  bash\n  200 ttys002  openhands --headless -t test\n",
          stderr: "",
        });
      }
      return Promise.reject(new Error("unexpected"));
  ...
  it("does not match similar process names like openhands-extra", async () => {
    mockExecFileAsync.mockImplementation((cmd: string, args: string[]) => {
      if (cmd === "tmux" && args[0] === "list-panes") {
        return Promise.resolve({ stdout: "/dev/ttys001\n", stderr: "" });
      }
      if (cmd === "ps") {
        return Promise.resolve({
          stdout: "  PID TT ARGS\n  100 ttys001  /usr/bin/openhands-extra\n",
          stderr: "",
        });
      }
      return Promise.reject(new Error("unexpected"));
...
describe("getSessionInfo", () => {
  ...
  it("returns null (OpenHands has no JSONL introspection yet)", async () => {
...
describe("getActivityState", () => {
  ...
  it("returns null (OpenHands has no session introspection yet)", async () => {
