...
async function postWithRetry(
  ...
  // When set, the next iteration skips its standard exponential backoff to
  // avoid double-delay after a 429 Retry-After wait.
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    try {
      ...
      // Set skipNextBackoff so the following iteration doesn't also apply
      // exponential backoff after we already waited for Retry-After.
