...
function mockTmuxWithProcess(processName = "goose", tty = "/dev/ttys001", pid = 12345) {
...
describe("plugin manifest & exports", () => {
  it("has correct manifest metadata", () => {
    expect(manifest).toEqual({
      name: "goose",
      slot: "agent",
      description: "Agent plugin: Goose",
      version: "0.1.0",
    });
  });
  ...
  it("create() returns an agent with correct name and processName", () => {
    ...
    expect(agent.processName).toBe("goose");
  });
...
describe("getLaunchCommand", () => {
  ...
  it("generates base command 'goose run' with no extra flags", () => {
    const cmd = agent.getLaunchCommand(makeLaunchConfig());
    expect(cmd).toBe("goose run");
  });

  it("uses -t flag for prompt (headless task mode)", () => {
  ...
  it("does not include permissions flag (task mode is inherently autonomous)", () => {
  ...
  it("generates command with prompt", () => {
    ...
    expect(cmd).toBe("goose run -t 'Hello'");
  });

  it("omits -t flag when no prompt provided", () => {
    const cmd = agent.getLaunchCommand(makeLaunchConfig());
...
describe("isProcessRunning", () => {
  ...
  it("returns true when goose is found on tmux pane TTY", async () => {
    mockTmuxWithProcess("goose");
    expect(await agent.isProcessRunning(makeTmuxHandle())).toBe(true);
  });

  it("returns false when no goose process on tmux pane TTY", async () => {
  ...
  it("finds goose on any pane in multi-pane session", async () => {
    mockExecFileAsync.mockImplementation((cmd: string, args: string[]) => {
      if (cmd === "tmux" && args[0] === "list-panes") {
  ...
  it("does not match similar process names like goose-extra", async () => {
    mockExecFileAsync.mockImplementation((cmd: string, args: string[]) => {
      if (cmd === "tmux" && args[0] === "list-panes") {
        return Promise.resolve({ stdout: "/dev/ttys001\n", stderr: "" });
      }
      if (cmd === "ps") {
        return Promise.resolve({
          stdout: "  PID TT ARGS\n  100 ttys001  /usr/bin/goose-extra\n",
          stderr: "",
        });
      }
      return Promise.reject(new Error("unexpected"));
...
describe("getSessionInfo", () => {
  ...
  it("returns null (Goose has no JSONL introspection yet)", async () => {
...
describe("getActivityState", () => {
  ...
  it("returns null (Goose has no session introspection yet)", async () => {
