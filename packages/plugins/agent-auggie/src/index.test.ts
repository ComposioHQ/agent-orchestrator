...
describe("plugin manifest & exports", () => {
  it("has correct manifest metadata", () => {
    expect(manifest).toEqual({
      name: "auggie",
      slot: "agent",
      description: "Agent plugin: Auggie",
      version: "0.1.0",
    });
  });
...
describe("getLaunchCommand", () => {
  ...
  it("escapes single quotes in prompt using POSIX method", () => {
    ...
    expect(cmd).toContain("--print 'it'\\''s a test'");
  });
  ...
  it("combines permissions and prompt", () => {
    ...
    expect(cmd).toBe("auggie --quiet --print 'Hello'");
  });
...
describe("isProcessRunning", () => {
  ...
  it("returns false when no auggie process on tmux pane TTY", async () => {
  ...
  it("finds auggie on any pane in multi-pane session", async () => {
    mockExecFileAsync.mockImplementation((cmd: string, args: string[]) => {
      if (cmd === "tmux" && args[0] === "list-panes") {
  ...
  it("does not match similar process names like auggie-extra", async () => {
    mockExecFileAsync.mockImplementation((cmd: string, args: string[]) => {
      if (cmd === "tmux" && args[0] === "list-panes") {
        return Promise.resolve({ stdout: "/dev/ttys001\n", stderr: "" });
      }
      if (cmd === "ps") {
        return Promise.resolve({
          stdout: "  PID TT ARGS\n  100 ttys001  /usr/bin/auggie-extra\n",
          stderr: "",
        });
      }
      return Promise.reject(new Error("unexpected"));
...
describe("getSessionInfo", () => {
  ...
  it("returns null (Auggie has no JSONL introspection yet)", async () => {
...
describe("getActivityState", () => {
  ...
  it("returns null (Auggie has no session introspection yet)", async () => {
