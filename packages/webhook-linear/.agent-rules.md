...
| When | Remove | Add |
|------|--------|-----|
| You start working on the issue | `agent-ready`, `needs-spec` | `agent-working` |
| CI fails on your PR | — | `ci-failed` |
| You fix CI and it passes | `ci-failed` | — |
| You open a PR | — | — (status change handled by orchestrator) |
| You need hardware for QA | — | `hardware` |
| You get stuck or blocked | `agent-working` | — (orchestrator detects stuck state) |
...
**IMPORTANT:** The FIRST thing you do when you start working is remove `agent-ready` and add `agent-working`. The LAST thing you do before finishing is confirm labels are clean.
