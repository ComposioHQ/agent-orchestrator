...
describe("listDashboardOrchestrators (issue #1048)", () => {
  ...
  it("prefers live orchestrators over more-recent terminal ones within a project (#1362)", () => {
    const killedLifecycle = createInitialCanonicalLifecycle("orchestrator", new Date("2026-04-20T12:00:00Z"));
    killedLifecycle.session.state = "terminated";
    killedLifecycle.session.reason = "runtime_missing";
    killedLifecycle.runtime.state = "missing";
    ...
    // bestByProject picks only one orchestrator per project — the live one wins
    expect(result).toHaveLength(1);
