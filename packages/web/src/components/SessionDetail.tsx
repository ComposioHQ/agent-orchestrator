...
function TerminalExitedPlaceholder({
  ...
  return (
    <div className="terminal-exited-placeholder" style={{ height }}>
      <div className="flex flex-col items-center gap-3 text-center">
        <span className="terminal-exited-placeholder__text">{message}</span>
        {ctaHref ? (
          <a
            href={ctaHref}
            className="done-restore-btn hover:no-underline"
          >
            {ctaLabel ?? "Open current orchestrator"}
          </a>
        ) : null}
      </div>
    </div>
            ...
            {onRestore ? (
              ...
              >
                ...
                >
                  ...
                  <path d="M20 7v4a5 5 0 0 1-5 5H4" />
                </svg>
                Restore
              </button>
            ) : onKill ? (
...
export function SessionDetail({
  ...
  const headline = isOrchestrator ? "Orchestrator" : getSessionTitle(session);
  ...
  const replacementOrchestratorHref = useMemo(() => {
    if (!isOrchestrator || !terminalEnded) return null;
    if (!projectOrchestratorId || projectOrchestratorId === session.id) return null;
    return getProjectSessionHref(session.projectId, projectOrchestratorId);
  }, [isOrchestrator, terminalEnded, projectOrchestratorId, session.id, session.projectId]);
  ...
  if (!isMobile) {
          ...
          <main className="min-w-0">
            ...
            <section className="session-detail-terminal-wrap">
              ...
              {!showTerminal ? (
                ...
                <TerminalExitedPlaceholder
                  height={terminalHeight}
                  message={
                    replacementOrchestratorHref
                      ? "This orchestrator has already ended."
                      : "Terminal session has ended"
                  }
                  ctaHref={replacementOrchestratorHref}
                  ctaLabel="Open current orchestrator"
                />
              ) : (
                <DirectTerminal
    ...
    // When no projects are supplied, the parent (DashboardShell) already renders
    // the app chrome (header + sidebar). Skip the internal wrappers to avoid
    // duplicate headers and give the content the full available width.
    ...
    return (
      <div className="dashboard-app-shell">
        ...
        >
          ...
          <div className="dashboard-main dashboard-main--desktop">{mainContent}</div>
        </div>
      </div>
    );
  }
