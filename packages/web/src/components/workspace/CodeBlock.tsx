...
function findCodeChild(children: ReactNode): ReactElement<CodeElementProps> | null {
...
export function CodeBlock({ children }: CodeBlockProps) {
  ...
  // Mermaid is handled by the `code` component override — short-circuit so the
  // header wrapper doesn't appear around diagrams. Returning just `children`
  // lets ReactMarkdown render the code element, which the `code` override
  // swaps for <MermaidDiagram />.
  if (language === "mermaid") {
    return <>{children}</>;
  }
  ...
  const handleCopy = useCallback(async () => {
    ...
    try {
      ...
      // Swallow — user will see no confirmation and can retry.
    }
  }, [rawText]);
