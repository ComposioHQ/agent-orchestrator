...
/**
 * Parse unified diff from `git diff HEAD` (or similar).
 * Skips preamble; parses hunks and line types per plan rules.
 */
  ...
  while (i < rawLines.length) {
    ...
    while (i < rawLines.length && !rawLines[i].startsWith("@@")) {
      ...
      if (prefix === "+") {
        ...
        // Ignore unexpected lines inside hunk (e.g. malformed)
        i++;
        continue;
      }
      i++;
...
/** Untracked file: every line is an addition. */
