...
function DoneCard({
  ...
  const title =
    ...
    session.summary ||
    session.id;
  const isMerged = session.status === "merged" || session.pr?.state === "merged";
  const isTerminated = session.status === "killed" || session.status === "terminated";
  const isRestorable = !isMerged && (isTerminated || session.activity === "exited");
...
function DashboardInner({
  ...
  const showZeroProjectsEmptyState = allProjectsView && projects.length === 0;
  const currentProjectInfo = useMemo<ProjectInfo | null>(() => {
    if (allProjectsView) return null;
    if (projectId) {
      return (
    ...
    return null;
  }, [allProjectsView, projectId, projectName, projects]);
  ...
  const orchestratorHref = currentProjectOrchestrator
    ? getProjectSessionHref(
        currentProjectOrchestrator.projectId,
        currentProjectOrchestrator.id,
      )
    : null;
  ...
  const sidebarProjectSessions = useMemo(() => {
    if (!sidebarSessions) return sessions;
    if (!projectId) return sidebarSessions;

    const merged = new Map<string, DashboardSession>();
    ...
    return [...merged.values()];
  }, [projectId, sessions, sidebarSessions]);
  ...
  const handleSpawnOrchestrator = async (project: ProjectInfo) => {
    if (project.degraded) {
      setSpawnErrors((current) => ({
        ...current,
        [project.id]: project.degradedReason || `${project.name} has an unresolved config error`,
      }));
      return;
    }
      ...
      <>
        ...
        <div className="dashboard-app-shell">
          {showSidebar && (
            <ProjectSidebar
              projects={projects}
              sessions={sidebarProjectSessions}
        ...
        >
          ...
          <div className="border-t border-[var(--color-border-subtle)] pt-3">
            <div className="flex items-center justify-between gap-3">
              ...
              {orchestrator ? (
                <a
                  href={getProjectSessionHref(orchestrator.projectId, orchestrator.id)}
                ...
                <button
                  ...
                  disabled={spawningProjectIds.includes(project.id) || project.degraded}
