...
export interface RecoveryResult {
  ...
  reason?: string;
  /** HTTP status from the restore route, present on non-2xx. */
  status?: number;
}
 ...
 * `mux-websocket.ts`).
 */
export function createSessionRecoverer(
  nextPort: string,
): (sessionId: string) => Promise<RecoveryResult> {
  const baseUrl = `http://localhost:${nextPort}`;

  return async (sessionId: string): Promise<RecoveryResult> => {
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), RECOVERY_TIMEOUT_MS);

    try {
      const res = await fetch(
        `${baseUrl}/api/sessions/${encodeURIComponent(sessionId)}/restore`,
        {
          method: "POST",
