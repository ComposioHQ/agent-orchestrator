...
describe("setup openclaw command", () => {
  ...
  describe("non-interactive mode", () => {
    ...
    it("reads token from OPENCLAW_HOOKS_TOKEN env var", async () => {
      ...
      // Non-interactive mode skips pre-validation; just writes config
    ...
    it("reads URL from OPENCLAW_GATEWAY_URL env var", async () => {
      ...
      // Non-interactive mode skips pre-validation; config is still written
      expect(mockValidateToken).not.toHaveBeenCalled();
      expect(mockWriteFileSync).toHaveBeenCalled();
    });
    ...
    it("skips token validation and writes config in non-interactive mode", async () => {
      ...
      // the token yet on a fresh install)
      expect(mockValidateToken).not.toHaveBeenCalled();
      expect(mockWriteFileSync).toHaveBeenCalled();
    });
  });
