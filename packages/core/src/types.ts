...
export interface Agent {
   ...
   *
   * `config` carries the per-session resolved launch parameters (permissions, model,
   * subagent) — plugins must prefer these over `config.projectConfig.agentConfig`,
   * which only reflects project defaults. See #1475.
   */
  getRestoreCommand?(session: Session, config: AgentLaunchConfig): Promise<string | null>;
