...
// syntheticPromptMarkers open a user turn Claude Code injected itself: the
// caveat prepended to local command output, a system reminder, a hook result,
// or the echo of a slash command. They are recorded as user messages but the
// user did not type them, so treating one as a prompt would put machine text in
// the sidebar and send it to a classifier as if it were a request.
var syntheticPromptMarkers = []string{
...
// isSyntheticPrompt reports whether a recorded user turn was injected by the
// CLI rather than typed by the user.
func isSyntheticPrompt(text string) bool {
	trimmed := strings.TrimSpace(text)
	if trimmed == "" {
		return true
	}
	for _, marker := range syntheticPromptMarkers {
