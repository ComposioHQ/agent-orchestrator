...
func (s *Service) EditMessage(
	...
	replacement := newController(id, conversation, generation, source.harness, source.codexProfileID, provider, s.store, s.activity, s.log, s.newID, s.now, s.onAccountChanged, s.onCapacityChanged, s.onUsageLimited)
...
func (s *Service) restoreClosedSourceController(
	...
	replacement := newController(
		...
		s.onAccountChanged, s.onCapacityChanged, s.onUsageLimited,
	)
