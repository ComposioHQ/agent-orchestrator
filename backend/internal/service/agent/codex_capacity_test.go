...
func TestCodexCapacitySingleFlight(t *testing.T) {
	...
	factory := &fakeCodexAccountFactory{capabilities: domain.CodexProfileCapabilities{AccountRead: supported, CapacityRead: supported}, open: func(ports.CodexAccountProfile) ports.CodexAccountClient { return client }}
	manager := newCodexProfileManager(context.Background(), t.TempDir(), t.TempDir(), factory, nil)
	manager.now = func() time.Time { return now }
	manager.capacity.now = manager.now
	manager.catalog.updateSnapshot(codexExistingProfileID, func(snapshot *domain.CodexProfileSnapshot) {
		...
		snapshot.AuthMethod = domain.CodexAuthMethodChatGPT
	})
	...
	if got := manager.capacity.snapshot(codexExistingProfileID); got.State != domain.CodexCapacityAvailable {
...
func TestCodexCapacityRequestCancellationDoesNotCancelSharedRead(t *testing.T) {
	...
	factory := &fakeCodexAccountFactory{capabilities: domain.CodexProfileCapabilities{CapacityRead: supported}, open: func(ports.CodexAccountProfile) ports.CodexAccountClient { return client }}
	manager := newCodexProfileManager(context.Background(), t.TempDir(), t.TempDir(), factory, nil)
	manager.now = func() time.Time { return now }
	manager.capacity.now = manager.now
	record := codexCapacityTestRecord(t.TempDir(), "existing", domain.CodexProfileSourceExisting, now)
...
func TestCodexCapacityReadsShareTwoProcessLimit(t *testing.T) {
	...
	for _, id := range []string{"one", "two", "three"} {
		record := codexCapacityTestRecord(t.TempDir(), id, domain.CodexProfileSourceManaged, now)
		go func() {
			_, _ = manager.capacity.ensureOne(context.Background(), record, factory.capabilities, false)
			done <- struct{}{}
		}()
...
func codexCapacityTestRecord(home, id string, source domain.CodexProfileSource, now time.Time) codexProfileRecord {
	return codexProfileRecord{Home: home, Snapshot: domain.CodexProfileSnapshot{
		ID: id, Source: source, Status: domain.CodexProfileStatusValid,
