...
func runSessions(t *testing.T, newHarness Factory) {
	...
	t.Run("concurrent creates allocate unique project numbers", func(t *testing.T) {
		h := newHarness(t)
		ctx := h.ctx()
		mustUpsertProject(t, h, newProject("acme", "/repos/acme"))
		const workers = 8
		...
		for i := 0; i < workers; i++ {
			...
			go func(i int) {
				defer wg.Done()
				rec := newSession("acme")
				rec.IssueID = domain.IssueID(fmt.Sprintf("issue-%d", i))
				created, err := h.Sessions.CreateSession(ctx, rec)
				if err != nil {
					errs <- err
					return
				}
		...
		seen := make(map[domain.SessionID]bool, workers)
		for id := range ids {
			if seen[id] {
		...
		if len(seen) != workers {
			t.Fatalf("created %d sessions, want %d", len(seen), workers)
		}
	})
