...
const (
	...
	sessionsHarnessCheckWithMuseKimchiPrimeAgentOMPDeepSeek   = `CHECK (harness IN ('', 'claude-code', 'codex', 'aider', 'opencode', 'grok', 'droid', 'amp', 'agy', 'crush', 'cursor', 'qwen', 'copilot', 'goose', 'auggie', 'continue', 'devin', 'cline', 'kimi', 'muse', 'kiro', 'kilocode', 'vibe', 'pi', 'kimchi', 'prime-agent', 'autohand', 'omp', 'deepseek-harness', 'fake'))`
	sessionsHarnessCheckWithMuseQMKimchiPrimeAgentOMPDeepSeek = `CHECK (harness IN ('', 'claude-code', 'codex', 'aider', 'opencode', 'grok', 'droid', 'amp', 'agy', 'crush', 'cursor', 'qwen', 'copilot', 'goose', 'auggie', 'continue', 'devin', 'cline', 'kimi', 'muse', 'kiro', 'kilocode', 'vibe', 'pi', 'kimchi', 'prime-agent', 'autohand', 'omp', 'deepseek-harness', 'qm', 'fake'))`
)
...
func reconcileHarnessConstraint(db *sql.DB) error {
	...
	if !needsMuse && !needsKimchi && !needsPrimeAgent && !needsOMP && !needsDeepSeek {
		return nil
	}
	...
	if needsDeepSeek {
		repairs = append(repairs,
			replacement{sessionsHarnessCheckWithMuseKimchiPrimeAgentOMP, sessionsHarnessCheckWithMuseKimchiPrimeAgentOMPDeepSeek},
			replacement{sessionsHarnessCheckWithMuseQMKimchiPrimeAgentOMP, sessionsHarnessCheckWithMuseQMKimchiPrimeAgentOMPDeepSeek},
		)
	}
...
WHERE type = 'table' AND name = 'sessions'`,
	...
	if !strings.Contains(schema, "'deepseek-harness'") {
		return fmt.Errorf("schema repair: sessions harness constraint is missing DeepSeek Harness and did not match known pre-DeepSeek-Harness schema")
	}
	return nil
}
