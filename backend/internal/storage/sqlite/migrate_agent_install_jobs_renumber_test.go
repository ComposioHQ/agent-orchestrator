...
func TestMigrateRepairsAgentInstallJobsFromLegacy0119(t *testing.T) {
	...
	contents, err := migrationsFS.ReadFile("migrations/0120_agent_install_jobs.sql")
	if err != nil {
		t.Fatalf("read agent install jobs migration: %v", err)
	}
	gooseMu.Lock()
	goose.SetBaseFS(fstest.MapFS{
		"migrations/0119_agent_install_jobs.sql": &fstest.MapFile{Data: contents},
	})
	...
	for _, version := range []int64{119, 120} {
		var applied int
		if err := db.QueryRow(`
SELECT COALESCE((
