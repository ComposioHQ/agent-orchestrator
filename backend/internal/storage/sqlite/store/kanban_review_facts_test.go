...
// A push that AO has seen but whose checks it has not observed yet leaves the
// aggregate ci_state describing the previous commit. The PR facts must say so,
// so the Kanban reducer can ignore a verdict about code that is no longer the
// PR.
func TestListPRFactsForSessionFlagsCIRecordedForAnEarlierHead(t *testing.T) {
	s := newTestStore(t)
	ctx := context.Background()
	seedProject(t, s, "mer")
	r, _ := s.CreateSession(ctx, sampleRecord("mer"))
	...
	write := func(url, headSHA string, checks []domain.PullRequestCheck) {
		t.Helper()
		if err := s.WriteSCMObservation(ctx, domain.PullRequest{
			URL: url, SessionID: r.ID, Number: 1, CI: domain.CIFailing,
			HeadSHA: headSHA, UpdatedAt: now, ObservedAt: now,
		}, checks, nil, nil, nil, ports.ReviewWritePreserve); err != nil {
			t.Fatalf("write %s: %v", url, err)
		}
	}
	failed := func(sha string) []domain.PullRequestCheck {
		return []domain.PullRequestCheck{
			{Name: "build", CommitHash: sha, Status: domain.PRCheckFailed, CreatedAt: now},
		}
	}
	write("pr/no-checks", "head1", nil)
	write("pr/current", "head1", failed("head1"))
	// head1's checks are still the only ones stored when head2 lands.
	write("pr/pushed", "head1", failed("head1"))
	write("pr/pushed", "head2", nil)
	...
	for _, f := range facts {
		got[f.URL] = f.CIAtCurrentHead
	}
	want := map[string]bool{"pr/no-checks": true, "pr/current": true, "pr/pushed": false}
	for url, wantCurrent := range want {
		if got[url] != wantCurrent {
			t.Fatalf("%s ciAtCurrentHead = %v, want %v (all: %+v)", url, got[url], wantCurrent, got)
		}
	}
}
