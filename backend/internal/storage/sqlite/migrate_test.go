...
var expectedUsageTableColumns = map[string][]string{
	...
	"model_usage_events": {
		...
		// 0113 appends: ALTER TABLE has no way to place a column mid-row, and a
...
// The provider detail tables were the shape 0112 replaced with one bounded
...
// TestUsageMeasurementMigrationFoldsCostsAndRetiresDetailTables covers 0112
// directly: a pre-0112 profile is seeded through the migrations that shipped
...
// 0113. Every row attributed before the column existed came from the transcript
