...
var expectedUsageTableColumns = map[string][]string{
	...
	"model_usage_events": {
		...
		"billing_provider_id", "uncached_input_cost_nanos", "cache_read_cost_nanos",
...
func TestUsageCostMigrationKeepsLegacyRowsUnattributedAndUnpriced(t *testing.T) {
	...
	var billingProviderID sql.NullString
	var uncachedCost, readCost, writeCost, outputCost, totalCost sql.NullInt64
	if err := db.QueryRow(`
SELECT ub.provider_hint, mue.billing_provider_id,
...
WHERE mue.source_event_key = 'migration-event'`).Scan(
		&providerHint, &billingProviderID,
	...
	if providerHint != "" || pricingVersion != "" || billingProviderID.Valid ||
		...
		t.Fatalf("legacy defaults = hint:%q billing:%v costs:%v/%v/%v/%v/%v version:%q",
			providerHint, billingProviderID,
			uncachedCost, readCost, writeCost, outputCost, totalCost, pricingVersion)
	}
