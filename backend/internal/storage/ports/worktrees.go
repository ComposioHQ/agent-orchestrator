...
// not stored: that is probed, not remembered.
type SessionWorktreeStore interface {
	...
	// ListSessionWorktrees returns every repo worktree for a session, root
	// first, then children by repo name.
	...
	// idempotent: a session with no rows is not an error.
