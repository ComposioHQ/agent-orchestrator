...
//
// It stores facts, never display state. Status is derived at read time from
// activity_state, is_terminated, and PR facts (see domain.DeriveStatus), so
// there is no SetStatus here and there must never be one. PR, check, review,
// and comment facts belong to the SCM observation stores and are joined above
// this port, not inside it.
//
...
type SessionStore interface {
	...
	// reads, so it must not fan out one query per project.
	...
	// id, project, per-project number, and created_at are immutable.
	...
	// preview revision, so a repeated request for an unchanged URL still
	// reaches the client as a navigation.
	...
	// no workspace, no runtime handle, no agent session id, no prompt, no
	// handoff metadata, and not terminated. Rows with observable spawn output
	// are immutable, which is what makes the no-resurrection guarantee hold.
	// deleted=false means the row was absent or had progressed past seed state;
	// callers fall back to marking the session terminated.
