...
type APIDeps struct {
	...
	PrerequisiteRunner  controllers.PrerequisiteRunner
...
type API struct {
	...
	prerequisites *controllers.PrerequisitesController
...
func NewAPI(cfg config.Config, deps APIDeps) *API {
	return &API{
		...
		prerequisites: &controllers.PrerequisitesController{Runner: deps.PrerequisiteRunner},
