...
type APIDeps struct {
	...
	CodexProfiles      controllers.CodexProfileService
...
type API struct {
	...
	codexProfiles *controllers.CodexProfilesController
...
func NewAPI(cfg config.Config, deps APIDeps) *API {
	return &API{
		...
		codexProfiles: &controllers.CodexProfilesController{Svc: deps.CodexProfiles},
		projects: &controllers.ProjectsController{
			Mgr: deps.Projects,
		},
		sessions: &controllers.SessionsController{
...
func (a *API) Register(root chi.Router) {
	...
	root.Route("/api/v1", func(r chi.Router) {
		...
		a.codexProfiles.RegisterStreams(r)
