...
func (r *Runtime) Attach(ctx context.Context, handle ports.RuntimeHandle, rows, cols uint16) (ports.Stream, error) {
	...
	// Resolve the session's socket once and use it for both calls below: the
	// option has to land on the very server we are about to attach to.
	socketName := r.socketForSession(ctx, id)
	...
	// Those pre-existing sessions are also exactly the ones socketForSession
	// routes back to tmux's legacy default socket, which is why this goes
	// through the resolved socket rather than r.run.
	...
	_, _ = r.runOnSocket(ctx, socketName, setDetachOnDestroyOnArgs(id)...)
