...
//
// The test drives its own private tmux server (TMUX_TMPDIR + `-f /dev/null`) so
// it reproduces on a default machine and leaves the developer's own server and
// config alone.
func TestRuntimeIntegrationRestartUnderCustomBaseIndex(t *testing.T) {
	...
	// A server exits as soon as it owns no sessions, so the keep-alive session
	// is what holds it open long enough to set the options. `-f /dev/null` skips
	// the developer's tmux.conf, making the indices below the only ones in play.
	if out, err := exec.Command("tmux", "-f", "/dev/null", "new-session", "-d", "-s", "ao-baseindex-keepalive", "sh", "-c", "sleep 120").CombinedOutput(); err != nil {
