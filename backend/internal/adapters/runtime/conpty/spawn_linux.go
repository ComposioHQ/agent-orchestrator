...
func defaultSpawnHost(ctx context.Context, sessionID, cwd string, argv []string, env map[string]string) (string, int, error) {
	...
	if err != nil {
		return "", 0, fmt.Errorf("linux pty spawn: resolve executable: %w", err)
	}
	...
	if err != nil {
		return "", 0, fmt.Errorf("linux pty spawn: stdout pipe: %w", err)
	}
	stderrFile, err := os.CreateTemp("", "ao-pty-host-stderr-*.log")
	if err != nil {
		_ = stdout.Close()
		_ = childStdout.Close()
		return "", 0, fmt.Errorf("linux pty spawn: stderr file: %w", err)
	}
	...
	if err := cmd.Start(); err != nil {
		...
		_ = childStdout.Close()
		_ = stderrFile.Close()
		return "", 0, fmt.Errorf("linux pty spawn: start: %w", err)
	}
	_ = childStdout.Close()
	...
	go func() {
		...
		msg := "linux pty spawn: pty-host exited without printing READY"
		if diag := strings.TrimSpace(readLinuxSpawnDiagnostics(stderrFile)); diag != "" {
			msg += ": " + diag
		}
	...
	case result := <-readyC:
		if result.err != nil {
			stopFailedLinuxSpawn(cmd, stdout, stderrFile)
			return "", 0, result.err
		}
		pid := cmd.Process.Pid
		_ = stdout.Close()
		...
		// still lets it survive daemon exit, after which the init system adopts it.
		go func() { _ = cmd.Wait() }()
		return result.addr, pid, nil
	case <-timer.C:
		stopFailedLinuxSpawn(cmd, stdout, stderrFile)
		return "", 0, fmt.Errorf("linux pty spawn: pty-host startup timeout (%s)", spawnReadyTimeout)
	case <-ctx.Done():
		stopFailedLinuxSpawn(cmd, stdout, stderrFile)
		return "", 0, ctx.Err()
	}
...
func readLinuxSpawnDiagnostics(stderr *os.File) string {
...
func stopFailedLinuxSpawn(cmd *exec.Cmd, stdout, stderr *os.File) {
	_ = stdout.Close()
	_ = stderr.Close()
	if cmd.Process != nil {
		_ = cmd.Process.Kill()
