...
func TestReadHistoryReplaysTypedProviderErrorBeforeCompletionWithStableIdentity(t *testing.T) {
	const response = `{"thread":{"id":"thread-1","turns":[{` +
		`"id":"turn-a","status":"failed","error":{` +
	...
	for read := 0; read < 2; read++ {
		srv.reply("thread/read", response)
		events, err := conv.ReadHistory(context.Background())
		if err != nil {
			t.Fatalf("ReadHistory %d: %v", read, err)
		}
		if len(events) != 3 ||
			events[0].Kind != ports.ChatEventTurnStarted ||
			events[1].Kind != ports.ChatEventError ||
			events[2].Kind != ports.ChatEventTurnCompleted {
			t.Fatalf("events = %#v, want start, provider error, completion", events)
		}
		errorEvent := events[1]
		if errorEvent.ProviderEventID != "codex-history:thread-1:turn-a:error" ||
			errorEvent.ProviderConversationID != "thread-1" ||
			errorEvent.ProviderTurnID != "turn-a" {
			t.Fatalf("provider error identity = %+v", errorEvent)
		}
		if errorEvent.ErrorInfo == nil ||
			errorEvent.ErrorInfo.Headline != "Reconnecting... [1/5]" ||
			errorEvent.ErrorInfo.Detail != "stream disconnected before completion: You have no credits remaining. Add credits to continue using the API at https://platform.openai.com/settings/organization/billing" ||
			errorEvent.ErrorInfo.Action != ports.ChatRecoveryActionOpenAIBilling {
			t.Fatalf("provider error info = %+v", errorEvent.ErrorInfo)
		}
		if events[2].Err == nil || events[2].Err.Error() != "Reconnecting... [1/5]" {
			t.Fatalf("completion error = %v", events[2].Err)
		}
		...
		for i, event := range events {
			...
			if event.ProviderEventID == "" || seen[event.ProviderEventID] {
				t.Fatalf("event %d has missing or duplicate identity %q", i, event.ProviderEventID)
			}
		...
		for i := range ids {
			if ids[i] != firstIDs[i] {
				t.Fatalf("read identity %d = %q, first read %q", i, ids[i], firstIDs[i])
			}
		}
	}
}

func TestReadHistoryProviderErrorsStayReadableButFailClosed(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:       "unknown error variant",
			errorJSON:  `{"message":"Reconnecting","codexErrorInfo":{"futureVariant":{"code":"x"}},"additionalDetails":"` + positive + `"}`,
			wantDetail: positive,
		},
		{
			name:       "malformed disconnect status",
			errorJSON:  `{"message":"Reconnecting","codexErrorInfo":{"responseStreamDisconnected":{"httpStatusCode":"429"}},"additionalDetails":"` + positive + `"}`,
			wantDetail: positive,
		},
		{
			name:       "second union variant",
			errorJSON:  `{"message":"Reconnecting","codexErrorInfo":{"responseStreamDisconnected":{"httpStatusCode":429},"futureVariant":{}},"additionalDetails":"` + positive + `"}`,
			wantDetail: positive,
		},
		{
			name:       "unknown disconnect field",
			errorJSON:  `{"message":"Reconnecting","codexErrorInfo":{"responseStreamDisconnected":{"httpStatusCode":429,"future":true}},"additionalDetails":"` + positive + `"}`,
			wantDetail: positive,
		},
		{
			name:       "provider action URL",
			errorJSON:  `{"message":"Reconnecting","codexErrorInfo":{"responseStreamDisconnected":{"httpStatusCode":429}},"additionalDetails":"` + positive + `","actionUrl":"https://evil.example/billing"}`,
			wantDetail: positive,
		},
		{
			name:       "official provider action URL is still untrusted",
			errorJSON:  `{"message":"Reconnecting","codexErrorInfo":{"responseStreamDisconnected":{"httpStatusCode":429}},"additionalDetails":"` + positive + `","actionUrl":"https://platform.openai.com/settings/organization/billing"}`,
			wantDetail: positive,
		},
		{
			name:       "unknown TurnError field",
			errorJSON:  `{"message":"Reconnecting","codexErrorInfo":{"responseStreamDisconnected":{"httpStatusCode":429}},"additionalDetails":"` + positive + `","future":true}`,
			wantDetail: positive,
		},
		{
			name:      "malformed additional detail",
			errorJSON: `{"message":"Reconnecting","codexErrorInfo":{"responseStreamDisconnected":{"httpStatusCode":429}},"additionalDetails":429}`,
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if len(events) != 3 || events[1].Kind != ports.ChatEventError || events[2].Kind != ports.ChatEventTurnCompleted {
				t.Fatalf("events = %#v, want readable error before completion", events)
			}
			if events[1].ErrorInfo == nil ||
				events[1].ErrorInfo.Headline != "Reconnecting" ||
				events[1].ErrorInfo.Detail != tc.wantDetail {
				t.Fatalf("readable error = %+v", events[1].ErrorInfo)
			}
			if events[1].ErrorInfo.Action != "" {
				t.Fatalf("untrusted history error minted action %q", events[1].ErrorInfo.Action)
			}
		})
	}
}
