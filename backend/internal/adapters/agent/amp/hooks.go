...
func ampSystemPromptPluginSource(inline, file string) string {
	...
	b.WriteString("  if (sessionID !== activeThreadID) return;\n")
	...
	b.WriteString("    callHookSync(amp, \"session-start\", { session_id: event.thread.id });\n")
	b.WriteString("  });\n")
	...
	b.WriteString("    if (event.thread.id === activeThreadID) callHookSync(amp, \"stop\", { session_id: event.thread.id, status: event.status });\n")
	b.WriteString("  });\n")
	b.WriteString("}\n")
	return b.String()
}
