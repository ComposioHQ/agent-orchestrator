...
func ampSystemPromptPluginSource(inline, file string) string {
	...
	b.WriteString("  if (amp.activeThread?.id !== sessionID || sessionID !== activeThreadID) return;\n")
	...
	b.WriteString("  if (amp.activeThread?.id !== thread.id) return;\n")
	...
	b.WriteString("    if (event.thread.id === amp.activeThread?.id) callHookSync(amp, \"session-start\", { session_id: event.thread.id });\n")
	b.WriteString("  });\n")
	b.WriteString("  amp.on(\"agent.start\", async (event, ctx) => {\n")
	b.WriteString("    observeThread(amp, ctx.thread);\n")
	b.WriteString("    if (event.thread.id !== amp.activeThread?.id) return {};\n")
	...
	b.WriteString("    if (event.thread.id === amp.activeThread?.id && event.thread.id === activeThreadID) callHookSync(amp, \"stop\", { session_id: event.thread.id, status: event.status });\n")
	b.WriteString("  });\n")
	b.WriteString("}\n")
	return b.String()
}
