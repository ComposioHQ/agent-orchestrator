...
// for one Codex profile. Capacity is advisory and never participates in launch
// admission.
...
// profile. Reset-credit and raw provider payloads are deliberately absent.
...
// profile snapshot. It intentionally excludes buckets and account identity.
...
const (
	...
	CodexCapacityReasonProfileUnavailable = "capacity_profile_unavailable"
...
func CompactCodexCapacity(snapshot CodexCapacitySnapshot) CodexCapacitySummary {
	return CodexCapacitySummary{
		...
		UsedPercent: snapshot.UsedPercent, ResetsAt: snapshot.ResetsAt,
