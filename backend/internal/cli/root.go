...
func NewRootCommand(deps Deps) *cobra.Command {
	...
	root.AddCommand(newClaudeCodeLoginCommand(ctx))
...
func shouldEmitCLIInvocation(cmd *cobra.Command) bool {
	...
	case "ao daemon", "ao start", "ao completion", "ao help", "ao pty-host", "ao chat-host", "ao codex-login", "ao claude-code-login", "ao agent-process", "ao agent-process supervise":
		return false
	default:
		return true
	}
