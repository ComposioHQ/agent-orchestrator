...
func NewRootCommand(deps Deps) *cobra.Command {
	...
	root.AddCommand(newReviewTerminalCommand())
...
func shouldEmitCLIInvocation(cmd *cobra.Command) bool {
	...
	case "ao daemon", "ao start", "ao completion", "ao help", "ao pty-host", "ao review-terminal", "ao agent-process", "ao agent-process supervise":
		return false
	default:
		return true
	}
