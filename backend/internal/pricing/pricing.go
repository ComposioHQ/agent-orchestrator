...
// Cost estimates the USD cost of a turn. It returns the dollar figure and
// whether the model was priced; an unpriced model yields (0, false) so the
// caller records tokens with a zero cost rather than a wrong one. Negative
// token counts are clamped to zero so a malformed report cannot produce a
// negative cost.
