...
type SCMLinkService interface {
	StartInstall(ctx context.Context, tenant postgres.Tenant) (scm.InstallRedirect, string, error)
	CompleteInstall(ctx context.Context, params scm.CallbackParams) (domain.SCMInstallation, error)
	ListInstallations(ctx context.Context, tenant postgres.Tenant) ([]domain.SCMInstallation, error)
	ListRepositories(ctx context.Context, tenant postgres.Tenant, installationID string) ([]domain.SCMRepository, error)
	SyncInstallation(ctx context.Context, tenant postgres.Tenant, installationID string) ([]domain.SCMRepository, error)
	SetAllowlist(ctx context.Context, tenant postgres.Tenant, installationID string, repositoryFullNames []string) ([]domain.SCMRepository, error)
	Unlink(ctx context.Context, tenant postgres.Tenant, installationID string) error
}
...
type scmInstallStartRequest struct {
...
// tenantFromRequest binds the authenticated principal to the organization the
// caller named. Membership and role are enforced by RLS on every statement, so
// this only has to establish which organization is being addressed.
func (s *Server) tenantFromRequest(w http.ResponseWriter, r *http.Request, orgID string) (postgres.Tenant, bool) {
	principal, ok := principalFromContext(r.Context())
	if !ok {
		writeError(w, r, http.StatusUnauthorized, "unauthorized", "AUTH_REQUIRED", "valid AO access token required")
		return postgres.Tenant{}, false
	}
	...
	return postgres.Tenant{OrgID: orgID, UserID: principal.UserID}, true
}

func (s *Server) startSCMInstall(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Cache-Control", "no-store")
	var input scmInstallStartRequest
	if !decodeJSON(w, r, &input) {
		return
	}
	tenant, ok := s.tenantFromRequest(w, r, input.OrgID)
	if !ok {
		return
	}
	redirect, _, err := s.scm.Link.StartInstall(r.Context(), tenant)
	if err != nil {
...
func (s *Server) listSCMInstallations(w http.ResponseWriter, r *http.Request) {
	tenant, ok := s.tenantFromRequest(w, r, r.URL.Query().Get("orgId"))
	if !ok {
		return
	}
	installations, err := s.scm.Link.ListInstallations(r.Context(), tenant)
	if err != nil {
...
func (s *Server) listSCMRepositories(w http.ResponseWriter, r *http.Request) {
	...
	repositories, err := s.scm.Link.ListRepositories(r.Context(), tenant, chi.URLParam(r, "installationID"))
	if err != nil {
...
func (s *Server) syncSCMRepositories(w http.ResponseWriter, r *http.Request) {
	...
	repositories, err := s.scm.Link.SyncInstallation(r.Context(), tenant, chi.URLParam(r, "installationID"))
	if err != nil {
...
func (s *Server) setSCMAllowlist(w http.ResponseWriter, r *http.Request) {
	...
	repositories, err := s.scm.Link.SetAllowlist(
		r.Context(), tenant, chi.URLParam(r, "installationID"), input.Repositories,
	)
	if err != nil {
...
func (s *Server) unlinkSCMInstallation(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.scm.Link.Unlink(r.Context(), tenant, chi.URLParam(r, "installationID")); err != nil {
