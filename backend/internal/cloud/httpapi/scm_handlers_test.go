...
func (f *fakeLinkService) StartInstall(_ context.Context, tenant postgres.Tenant) (scm.InstallRedirect, string, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *fakeLinkService) ListInstallations(_ context.Context, tenant postgres.Tenant) ([]domain.SCMInstallation, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.tenants = append(f.tenants, tenant)
	if f.listErr != nil {
...
func (f *fakeLinkService) ListRepositories(_ context.Context, tenant postgres.Tenant, _ string) ([]domain.SCMRepository, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.tenants = append(f.tenants, tenant)
	if f.listErr != nil {
...
func (f *fakeLinkService) SyncInstallation(_ context.Context, tenant postgres.Tenant, _ string) ([]domain.SCMRepository, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.tenants = append(f.tenants, tenant)
	return f.repositories, nil
...
func (f *fakeLinkService) Unlink(_ context.Context, tenant postgres.Tenant, _ string) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.tenants = append(f.tenants, tenant)
	return f.unlinkErr
...
func TestSCMRoutesAreAbsentWithoutAGitHubApp(t *testing.T) {
	...
	response := doSCMRequest(t, server, http.MethodGet, "/api/cloud/v1/scm/github/installations?orgId=org-1", accessToken, nil)
...
func TestSCMRoutesRequireAuthentication(t *testing.T) {
	...
	for _, target := range []string{
		"/api/cloud/v1/scm/github/installations?orgId=org-1",
		"/api/cloud/v1/scm/github/installations/installation-1/repositories?orgId=org-1",
	} {
...
func TestStartInstallRequiresAnOrganization(t *testing.T) {
	link := &fakeLinkService{}
	server, accessToken := newSCMServer(t, SCMOptions{Link: link})
	response := doSCMRequest(t, server, http.MethodPost, "/api/cloud/v1/scm/github/installations", accessToken, []byte(`{}`))
	...
	if envelope.Code != "ORG_ID_REQUIRED" {
...
func TestStartInstallReturnsRedirectWithoutLeakingTheState(t *testing.T) {
	...
	response := doSCMRequest(
		t, server, http.MethodPost, "/api/cloud/v1/scm/github/installations", accessToken, []byte(`{"orgId":"org-1"}`),
	)
	...
	if link.tenants[0] != (postgres.Tenant{OrgID: "org-1", UserID: "user-1"}) {
...
func TestSetAllowlistPassesRepositoriesThrough(t *testing.T) {
	...
	response := doSCMRequest(
		...
		"/api/cloud/v1/scm/github/installations/installation-1/allowlist",
		accessToken,
		[]byte(`{"orgId":"org-1","repositories":["acme/widgets"]}`),
	)
	if response.Code != http.StatusOK {
		t.Fatalf("status = %d body = %s", response.Code, response.Body.String())
	}
	...
	for _, testCase := range cases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			response := doSCMRequest(
				...
				"/api/cloud/v1/scm/github/setup?state=abc&installation_id=55", "", nil,
			)
...
func TestInstallCallbackRedirectsToTheCompletionURL(t *testing.T) {
	...
	response := doSCMRequest(t, server, http.MethodGet, "/api/cloud/v1/scm/github/setup?state=abc&installation_id=55", "", nil)
	...
	failed := doSCMRequest(t, server, http.MethodGet, "/api/cloud/v1/scm/github/setup?state=abc&installation_id=55", "", nil)
...
func TestWebhookEndpointRequiresAValidSignature(t *testing.T) {
	...
	request := httptest.NewRequest(http.MethodPost, "/api/cloud/v1/scm/github/webhook", bytes.NewReader(body))
...
func TestWebhookEndpointIsAbsentWithoutASecret(t *testing.T) {
	...
	request := httptest.NewRequest(http.MethodPost, "/api/cloud/v1/scm/github/webhook", strings.NewReader("{}"))
	response := httptest.NewRecorder()
	server.Handler().ServeHTTP(response, request)
	if response.Code != http.StatusNotFound {
		t.Fatalf("status = %d", response.Code)
