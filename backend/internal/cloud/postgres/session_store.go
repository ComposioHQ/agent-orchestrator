...
func (s *Store) GetSession(ctx context.Context, id domain.SessionID) (domain.SessionRecord, bool, error) {
	...
	if err != nil {
		return domain.SessionRecord{}, false, fmt.Errorf("get session %s: %w", id, normalizeStorageError(err))
	}
...
func (s *Store) CreateSession(ctx context.Context, rec domain.SessionRecord) (domain.SessionRecord, error) {
	...
	err := s.inTenantWrite(ctx, func(tx pgx.Tx) error {
		// Serialize number allocation for one acting user's project. The unique
		// index remains the invariant; the advisory lock makes concurrent creates
		// wait rather than surfacing a retryable uniqueness failure to the API.
		if _, err := tx.Exec(ctx, `SELECT pg_advisory_xact_lock(hashtextextended(ao_current_org_id()::text || ':' || ao_current_user_id()::text || ':' || $1, 0))`, rec.ProjectID); err != nil {
			return err
		}
		var num int64
		if err := tx.QueryRow(
	...
	if err != nil {
		return domain.SessionRecord{}, fmt.Errorf("create session in %s: %w", rec.ProjectID, normalizeStorageError(err))
	}
	return rec, nil
}
...
func (s *Store) UpdateSession(ctx context.Context, rec domain.SessionRecord) error {
	...
	if err := s.inTenantWrite(ctx, func(tx pgx.Tx) error {
		...
		return fmt.Errorf("update session %s: %w", rec.ID, normalizeStorageError(err))
	}
	return nil
}
...
func (s *Store) UpdateSessionFromActivitySignal(ctx context.Context, rec domain.SessionRecord) (bool, error) {
	...
	if err := s.inTenantWrite(ctx, func(tx pgx.Tx) error {
		...
		return false, fmt.Errorf("update session %s from activity signal: %w", rec.ID, normalizeStorageError(err))
	}
...
func (s *Store) ClaimChatControllerGeneration(
	...
	if !claimed {
		return fmt.Errorf("claim chat controller generation for %s: %w", id, storageports.ErrStorageNotFound)
	}
	return nil
}
...
func (s *Store) updateSessionFields(
	...
	if err := s.inTenantWrite(ctx, func(tx pgx.Tx) error {
		...
		return false, fmt.Errorf("%s %s: %w", what, id, normalizeStorageError(err))
	}
	return updated > 0, nil
}
...
func (s *Store) DeleteSession(ctx context.Context, id domain.SessionID) (bool, error) {
	...
	if err := s.inTenantWrite(ctx, func(tx pgx.Tx) error {
		...
		return false, fmt.Errorf("delete seed session %s: %w", id, normalizeStorageError(err))
	}
