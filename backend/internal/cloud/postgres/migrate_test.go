...
func TestCloudMigrationsAreTenantScoped(t *testing.T) {
	...
	// The foundation's first eight versions are frozen. Later slices append
	// their own migrations, so assert the prefix rather than an exact count.
	if len(migrations) < 8 {
		t.Fatalf("migrations = %#v", migrations)
	}
	for index, version := range []int64{1, 2, 3, 4, 5, 6, 7, 8} {
