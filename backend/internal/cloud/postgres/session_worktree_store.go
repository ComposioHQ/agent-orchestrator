...
func (s *Store) UpsertSessionWorktree(ctx context.Context, row domain.SessionWorktreeRecord) error {
	...
	if err := s.inTenantWrite(ctx, func(tx pgx.Tx) error {
		...
		return fmt.Errorf("upsert session worktree %s/%s: %w", row.SessionID, row.RepoName, normalizeStorageError(err))
	}
	return nil
}
...
func (s *Store) GetSessionWorktree(
	...
	if err != nil {
		return domain.SessionWorktreeRecord{}, false,
			fmt.Errorf("get session worktree %s/%s: %w", sessionID, repoName, normalizeStorageError(err))
	}
...
func (s *Store) ListSessionWorktrees(
	...
	if err != nil {
		return nil, fmt.Errorf("list session worktrees for %s: %w", sessionID, normalizeStorageError(err))
	}
	return out, nil
}
...
func (s *Store) DeleteSessionWorktrees(ctx context.Context, sessionID domain.SessionID) error {
	if err := s.inTenantWrite(ctx, func(tx pgx.Tx) error {
		...
		return fmt.Errorf("delete session worktrees for %s: %w", sessionID, normalizeStorageError(err))
	}
	return nil
}
