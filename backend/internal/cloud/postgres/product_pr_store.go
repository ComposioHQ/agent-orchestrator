...
func (s *Store) WritePR(ctx context.Context, pr domain.PullRequest, checks []domain.PullRequestCheck, comments []domain.PullRequestComment) error {
	return s.writeProductPR(ctx, pr, checks, nil, nil, comments, ports.ReviewWritePreserve, true, false, false)
}
...
func (s *Store) WriteSCMObservation(ctx context.Context, pr domain.PullRequest, checks []domain.PullRequestCheck, reviews []domain.PullRequestReview, threads []domain.PullRequestReviewThread, comments []domain.PullRequestComment, mode ports.ReviewWriteMode) error {
	return s.writeProductPR(ctx, pr, checks, reviews, threads, comments, mode, false, false, false)
}
...
func (s *Store) ClaimPR(ctx context.Context, pr domain.PullRequest, checks []domain.PullRequestCheck, reviews []domain.PullRequestReview, threads []domain.PullRequestReviewThread, comments []domain.PullRequestComment, mode ports.ReviewWriteMode, allowActiveTakeover bool) (ports.ClaimOutcome, error) {
	...
	err := s.withProductTenantTx(ctx, pgx.TxOptions{}, func(tx pgx.Tx, identity tenant.Identity, recorder ports.ChangeEventRecorder) error {
		...
		return writeProductPRTx(ctx, tx, identity, recorder, pr, checks, reviews, threads, comments, mode, false, true, owner != "" && owner != pr.SessionID)
	})
	return outcome, err
}

func (s *Store) writeProductPR(ctx context.Context, pr domain.PullRequest, checks []domain.PullRequestCheck, reviews []domain.PullRequestReview, threads []domain.PullRequestReviewThread, comments []domain.PullRequestComment, mode ports.ReviewWriteMode, legacy, claim, ownerChanged bool) error {
	return s.withProductTenantTx(ctx, pgx.TxOptions{}, func(tx pgx.Tx, identity tenant.Identity, recorder ports.ChangeEventRecorder) error {
		canonical, err := resolveProductPRURL(ctx, tx, identity, pr.URL)
		if err != nil {
			return err
		}
		pr.URL = canonical
		return writeProductPRTx(ctx, tx, identity, recorder, pr, checks, reviews, threads, comments, mode, legacy, claim, ownerChanged)
	})
}

type priorPRFacts struct {
	exists  bool
...
func writeProductPRTx(ctx context.Context, tx pgx.Tx, identity tenant.Identity, recorder ports.ChangeEventRecorder, pr domain.PullRequest, checks []domain.PullRequestCheck, reviews []domain.PullRequestReview, threads []domain.PullRequestReviewThread, comments []domain.PullRequestComment, mode ports.ReviewWriteMode, legacy, claim, ownerChanged bool) error {
	...
	prior := priorPRFacts{}
	err := tx.QueryRow(ctx, `SELECT session_id, pr_state, ci_state, review_decision, mergeability
		FROM ao_pull_requests WHERE org_id=$1 AND owner_user_id=$2 AND url=$3 FOR UPDATE`, identity.OrgID, identity.UserID, pr.URL).
		Scan(&prior.session, &prior.state, &prior.ci, &prior.review, &prior.merge)
	if err == nil {
		prior.exists = true
		if !claim && prior.session != pr.SessionID {
			return fmt.Errorf("pr %s already belongs to session %s", pr.URL, prior.session)
		}
	...
	if err := writeProductPRChildren(ctx, tx, identity, recorder, pr, checks, reviews, threads, comments, mode, legacy); err != nil {
		return err
	}
	eventType := ports.ChangeEventPRCreated
	if prior.exists {
		eventType = ports.ChangeEventPRUpdated
	}
	if ownerChanged {
		eventType = ports.ChangeEventPRSessionChanged
	}
	if !prior.exists || ownerChanged || prior.state != state || prior.ci != defaultCI(pr.CI) || prior.review != defaultReview(pr.Review) || prior.merge != defaultMerge(pr.Mergeability) {
		payload, _ := json.Marshal(map[string]any{"url": pr.URL, "session": pr.SessionID, "state": state, "ci": defaultCI(pr.CI), "review": defaultReview(pr.Review), "mergeability": defaultMerge(pr.Mergeability)})
		if err := recorder.RecordChange(ctx, ports.PendingChangeEvent{ProjectID: productProjectID(pr.SessionID), SessionID: string(pr.SessionID), Type: eventType, Payload: payload}); err != nil {
			return err
		}
	}
	return nil
...
func writeProductPRChildren(ctx context.Context, tx pgx.Tx, id tenant.Identity, recorder ports.ChangeEventRecorder, pr domain.PullRequest, checks []domain.PullRequestCheck, reviews []domain.PullRequestReview, threads []domain.PullRequestReviewThread, comments []domain.PullRequestComment, mode ports.ReviewWriteMode, legacy bool) error {
	for _, c := range checks {
		var old domain.PRCheckStatus
		err := tx.QueryRow(ctx, `SELECT status FROM ao_pull_request_checks WHERE org_id=$1 AND owner_user_id=$2 AND pr_url=$3 AND name=$4 AND commit_hash=$5`, id.OrgID, id.UserID, pr.URL, c.Name, c.CommitHash).Scan(&old)
		...
		if fresh || old != defaultCheck(c.Status) {
			payload, _ := json.Marshal(map[string]any{"pr": pr.URL, "name": c.Name, "commit": c.CommitHash, "status": defaultCheck(c.Status)})
			if err := recorder.RecordChange(ctx, ports.PendingChangeEvent{ProjectID: productProjectID(pr.SessionID), SessionID: string(pr.SessionID), Type: ports.ChangeEventPRCheckRecorded, Payload: payload}); err != nil {
				return err
			}
		}
	}
	...
	if mode == ports.ReviewWriteReplace || mode == ports.ReviewWriteMerge {
		...
		for _, th := range threads {
			var oldResolved bool
			err := tx.QueryRow(ctx, `SELECT resolved FROM ao_pull_request_review_threads WHERE org_id=$1 AND owner_user_id=$2 AND pr_url=$3 AND thread_id=$4`, id.OrgID, id.UserID, pr.URL, th.ThreadID).Scan(&oldResolved)
			fresh := errors.Is(err, pgx.ErrNoRows)
			if err != nil && !fresh {
				return normalizeError(err)
			}
			...
			typ := ports.ChangeEventPRReviewThreadAdded
			if !fresh && th.Resolved && !oldResolved {
				typ = ports.ChangeEventPRReviewThreadResolved
			}
			if fresh || typ == ports.ChangeEventPRReviewThreadResolved {
				payload, _ := json.Marshal(map[string]any{"pr": pr.URL, "threadId": th.ThreadID, "resolved": th.Resolved})
				if err := recorder.RecordChange(ctx, ports.PendingChangeEvent{ProjectID: productProjectID(pr.SessionID), SessionID: string(pr.SessionID), Type: typ, Payload: payload}); err != nil {
					return err
				}
			}
		}
...
func (s *Store) GetPR(ctx context.Context, url string) (domain.PullRequest, bool, error) {
	...
	err := s.withProductTenantTx(ctx, pgx.TxOptions{AccessMode: pgx.ReadOnly}, func(tx pgx.Tx, id tenant.Identity, _ ports.ChangeEventRecorder) error {
...
func (s *Store) MarkPRCommentResolved(ctx context.Context, url, commentID string) (bool, error) {
	var changed bool
	err := s.withProductTenantTx(ctx, pgx.TxOptions{}, func(tx pgx.Tx, id tenant.Identity, _ ports.ChangeEventRecorder) error {
...
func (s *Store) UpdatePRLastNudgeSignature(ctx context.Context, url, value string) error {
	return s.withProductTenantTx(ctx, pgx.TxOptions{}, func(tx pgx.Tx, id tenant.Identity, _ ports.ChangeEventRecorder) error {
...
func productProjectID(id domain.SessionID) string {
	v := string(id)
