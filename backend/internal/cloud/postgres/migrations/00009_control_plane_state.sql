...
-- user. Hosted, they are user-chosen labels that are unique only inside a
-- tenant, so every key here is (org_id, ...) and every foreign key carries
-- org_id through. Keying on the AO id alone would silently merge two tenants
-- that both registered a project called "acme".
--
-- Scope is the organization. Row-level security requires both that the row's
-- org is the request's org and that the acting user is a member of it, so a
-- token for one org can never read another's rows even if the caller supplies
-- a matching id. Personal organizations, which every user gets at first
-- sign-in, make that the per-user boundary in practice. owner_user_id records
-- who registered a project or spawned a session; it is attribution, not a
-- second access-control key, so tightening v1 to owner-only visibility is a
...
CREATE TABLE ao_workspace_repos (
    ...
    git_status TEXT NOT NULL DEFAULT 'ready'
        CHECK (git_status IN ('ready', 'needs_init')),
    registered_at TIMESTAMPTZ NOT NULL,
    PRIMARY KEY (org_id, project_id, name),
    FOREIGN KEY (org_id, project_id)
        REFERENCES ao_projects(org_id, id) ON DELETE CASCADE
);
...
CREATE INDEX ao_sessions_org_project_num_idx
    ON ao_sessions(org_id, project_id, num);
...
CREATE TABLE ao_session_worktrees (
    ...
    FOREIGN KEY (org_id, session_id)
        REFERENCES ao_sessions(org_id, id) ON DELETE CASCADE
);
...
ALTER TABLE ao_workspace_repos ENABLE ROW LEVEL SECURITY;
ALTER TABLE ao_workspace_repos FORCE ROW LEVEL SECURITY;
CREATE POLICY ao_workspace_repos_tenant ON ao_workspace_repos
    FOR ALL
    USING (
        org_id = ao_current_org_id()
        AND ao_is_org_member(org_id, ao_current_user_id())
...
REVOKE ALL ON TABLE ao_projects, ao_workspace_repos, ao_sessions, ao_session_worktrees FROM PUBLIC;
...
DROP POLICY IF EXISTS ao_workspace_repos_tenant ON ao_workspace_repos;
...
DROP TABLE IF EXISTS ao_workspace_repos;
