...
func TestCompleteInstallLinksAndAllowlistsSelectedRepositories(t *testing.T) {
	fake := newFakeGitHub(t)
	...
	// Choosing specific repositories during installation is an explicit
	// allowlist act, so those repositories land allowed.
	...
	if len(repositories) != 2 || !repositories[0].Allowed {
...
func TestSetAllowlistIsAFullReplacementBoundedByVisibility(t *testing.T) {
	...
	repositories, err := service.SetAllowlist(context.Background(), tenant, installation.ID, []string{"Acme/Widgets"})
	if err != nil {
		t.Fatal(err)
	}
	...
	repositories, err = service.SetAllowlist(context.Background(), tenant, installation.ID, []string{"acme/docs"})
	if err != nil {
		t.Fatal(err)
	}
	for _, repository := range repositories {
	...
	if _, err := service.SetAllowlist(context.Background(), tenant, installation.ID, []string{"other/secret"}); !errors.Is(err, ErrRepositoryNotAllowed) {
		t.Fatalf("error = %v", err)
	}
	if _, err := service.SetAllowlist(context.Background(), tenant, installation.ID, []string{"garbage"}); !errors.Is(err, ErrInvalidRepository) {
		t.Fatalf("error = %v", err)
	}
}
...
func TestSyncInstallationNeverWidensTheAllowlist(t *testing.T) {
	...
	if _, err := service.SetAllowlist(context.Background(), tenant, installation.ID, nil); err != nil {
		t.Fatal(err)
	}
	...
	repositories, err := service.SyncInstallation(context.Background(), tenant, installation.ID)
	if err != nil {
		t.Fatal(err)
	}
...
func TestUnlinkScopesToTenant(t *testing.T) {
	...
	installations, err := service.ListInstallations(context.Background(), tenant)
	if err != nil {
		t.Fatal(err)
	}
