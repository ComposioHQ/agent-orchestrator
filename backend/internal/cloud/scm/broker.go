...
type BrokerStore interface {
	AllowedSCMRepository(ctx context.Context, identity tenant.Identity, fullName string) (domain.SCMInstallation, domain.SCMRepository, error)
	RecordSCMTokenGrant(ctx context.Context, identity tenant.Identity, grant domain.SCMTokenGrant) error
}
...
func (b *Broker) BrokerToken(ctx context.Context, request BrokerRequest) (BrokeredToken, error) {
	...
	identity := tenant.Identity{OrgID: request.OrgID, UserID: request.UserID}
	installation, allowed, err := b.store.AllowedSCMRepository(ctx, identity, repository)
	if err != nil {
	...
	// Only observer credentials are reused. Bootstrap credentials live for one
	// clone handoff and are not retained in the broker after that operation.
	cacheable := purpose == domain.TokenPurposeObserve
