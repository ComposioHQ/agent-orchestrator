...
func TestBrokerReusesLiveObserverTokenAndReMintsNearExpiry(t *testing.T) {
	fake := newFakeGitHub(t)
	store := newMemoryBrokerStore()
	store.allow("org-1", "acme/widgets", 55, 900)
	broker := newTestBroker(t, fake, store)
	...
	request := BrokerRequest{OrgID: "org-1", UserID: "user-1", Repository: "acme/widgets", Purpose: "observe"}
	...
	if len(fake.minted()) != 3 {
		t.Fatal("a push credential was served from the observer cache")
	}
}
...
func TestBrokerScopesLookupToRequestedTenant(t *testing.T) {
	...
	if len(store.tenants) != 1 || (store.tenants[0] != tenant.Identity{OrgID: "org-1", UserID: "user-9"}) {
