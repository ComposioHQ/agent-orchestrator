...
func TestCredentialFormattingAndSerializationRedactToken(t *testing.T) {
	secret := "ghs_super_secret_installation_token"
	credential := Credential{Username: CloneUsername, Token: []byte(secret), Repository: "acme/widgets"}
	...
	for name, rendered := range map[string]string{
		...
		"go string": fmt.Sprintf("%#v", credential),
		"json":      string(encoded),
		"slog":      logBuffer.String(),
	} {
		if strings.Contains(rendered, secret) || strings.Contains(rendered, "Z2hzX3N1cGVy") {
			t.Fatalf("%s leaked credential: %s", name, rendered)
		}
	}
}

// Bootstrap credentials are one-shot: issuing twice mints twice rather than
// retaining a live clone token in the broker cache after the first checkout.
func TestCloneCredentialsAreNeverCached(t *testing.T) {
	fake := newFakeGitHub(t)
	store := newMemoryBrokerStore()
	store.allow("org-1", "acme/widgets", 55, 900)
	broker := newTestBroker(t, fake, store)

	first, err := broker.IssueCloneCredential(context.Background(), testIdentity, "acme/widgets", "sandbox-abc")
	if err != nil {
		t.Fatal(err)
	}
	second, err := broker.IssueCloneCredential(context.Background(), testIdentity, "acme/widgets", "sandbox-abc")
	if err != nil {
		t.Fatal(err)
	}
	if len(fake.minted()) != 2 {
		t.Fatalf("minted %d credentials for two bootstraps", len(fake.minted()))
	}
	if bytes.Equal(first.Token, second.Token) {
		t.Fatal("a clone credential was retained across bootstraps")
	}
	if len(store.grants) != 2 {
		t.Fatalf("two bootstraps produced %d audit grants", len(store.grants))
	}
}
