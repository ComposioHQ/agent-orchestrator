...
// SCMObservationSignal is the normalized "something changed" hint a verified
...
type ObservationSink interface {
	ObserveSCMSignal(ctx context.Context, signal SCMObservationSignal) error
}
...
func observationSignal(event string, payload webhookPayload) (SCMObservationSignal, bool) {
	signal := SCMObservationSignal{
		Repository: strings.ToLower(strings.TrimSpace(payload.Repository.FullName)),
	}
	if signal.Repository == "" {
		return SCMObservationSignal{}, false
	}
	switch event {
