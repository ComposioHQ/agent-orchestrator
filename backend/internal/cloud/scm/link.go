...
type LinkStore interface {
	CreateSCMInstallState(ctx context.Context, identity tenant.Identity, stateHash []byte, expiresAt time.Time) error
	ConsumeSCMInstallState(ctx context.Context, stateHash []byte) (domain.SCMInstallationLink, error)
	UpsertSCMInstallation(ctx context.Context, identity tenant.Identity, installation domain.SCMInstallation) (domain.SCMInstallation, error)
	SCMInstallationByID(ctx context.Context, identity tenant.Identity, installationID string) (domain.SCMInstallation, error)
	ListSCMInstallations(ctx context.Context, identity tenant.Identity) ([]domain.SCMInstallation, error)
	DeleteSCMInstallation(ctx context.Context, identity tenant.Identity, installationID string) error
	SyncSCMRepositories(ctx context.Context, identity tenant.Identity, installationID string, repositories []domain.SCMRepository, allowNew bool) error
	ListSCMRepositories(ctx context.Context, identity tenant.Identity, installationID string) ([]domain.SCMRepository, error)
	SetSCMRepositoryAllowlist(ctx context.Context, identity tenant.Identity, installationID string, allowedExternalIDs []int64) error
}
...
func (s *LinkService) StartInstall(ctx context.Context, identity tenant.Identity) (InstallRedirect, string, error) {
	raw := make([]byte, 32)
...
func (s *LinkService) CompleteInstall(ctx context.Context, params CallbackParams) (domain.SCMInstallation, error) {
	...
	// Visibility is not authorization. Every discovered repository lands
	// denied until an AO organization admin explicitly updates the allowlist.
	if err := s.syncRepositories(ctx, identity, installation, false); err != nil {
		return domain.SCMInstallation{}, err
	}
...
func (s *LinkService) SyncInstallation(
	...
	installation, err := s.store.SCMInstallationByID(ctx, identity, installationID)
	if err != nil {
		return nil, err
	}
	if err := s.syncRepositories(ctx, identity, installation, false); err != nil {
	...
	return s.store.ListSCMRepositories(ctx, identity, installationID)
}
...
func (s *LinkService) syncRepositories(
	...
	return s.store.SyncSCMRepositories(ctx, identity, installation.ID, repositories, allowNew)
}
...
func (s *LinkService) SetAllowlist(
	...
	known, err := s.store.ListSCMRepositories(ctx, identity, installationID)
	if err != nil {
		return nil, err
	}
	...
	if err := s.store.SetSCMRepositoryAllowlist(ctx, identity, installationID, allowed); err != nil {
		return nil, err
	}
	return s.store.ListSCMRepositories(ctx, identity, installationID)
}
...
func (s *LinkService) ListInstallations(ctx context.Context, identity tenant.Identity) ([]domain.SCMInstallation, error) {
...
func (s *LinkService) ListRepositories(
	...
	if _, err := s.store.SCMInstallationByID(ctx, identity, installationID); err != nil {
		return nil, err
	}
	return s.store.ListSCMRepositories(ctx, identity, installationID)
}
...
func (s *LinkService) Unlink(ctx context.Context, identity tenant.Identity, installationID string) error {
	return s.store.DeleteSCMInstallation(ctx, identity, installationID)
}
