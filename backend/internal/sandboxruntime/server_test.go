...
func (fixedObserver) Snapshot(context.Context, ports.WorkspaceInfo) (ports.WorkspaceSnapshot, error) {
	return ports.WorkspaceSnapshot{Path: "/workspace", Branch: "main", HeadSHA: "abc", Dirty: true}, nil
}
func (fixedObserver) List(context.Context, ports.WorkspaceListRequest) (ports.WorkspaceListResult, error) {
	return ports.WorkspaceListResult{}, nil
}
func (fixedObserver) Read(context.Context, ports.WorkspaceReadRequest) (ports.WorkspaceReadResult, error) {
	return ports.WorkspaceReadResult{}, nil
}
func (fixedObserver) Watch(context.Context, ports.WorkspaceWatchRequest) (<-chan ports.WorkspaceEvent, error) {
	return make(chan ports.WorkspaceEvent), nil
}
func (fixedObserver) Diff(context.Context, ports.WorkspaceDiffRequest) (ports.WorkspaceDiffResult, error) {
	return ports.WorkspaceDiffResult{}, nil
}
func (fixedObserver) Blob(context.Context, ports.WorkspaceBlobRequest) (ports.WorkspaceBlobResult, error) {
	return ports.WorkspaceBlobResult{}, nil
}
...
func TestMuxRejectsGrantBoundToAnotherSession(t *testing.T) {
	target := testTarget()
	wrong := grantFor(target, ScopeTerminalRead)
	wrong.SessionID = "another-session"
	redeemer := &fakeRedeemer{grants: map[string]TicketGrant{"wrong-session": wrong}}
	server, pty := testServer(t, target, redeemer)
	httpServer := httptest.NewServer(server.http.Handler)
	defer httpServer.Close()
	defer pty.stop()
	offer, err := muxproto.Offer("wrong-session")
	if err != nil {
		t.Fatal(err)
	}
	wsURL := "ws" + strings.TrimPrefix(httpServer.URL, "http") + muxproto.Path
	...
	if err == nil {
		_ = conn.CloseNow()
		t.Fatal("ticket for another session was accepted")
	}
	if response == nil || response.StatusCode != http.StatusUnauthorized {
		t.Fatalf("session-binding status = %#v", response)
	}
}
...
func TestMuxRejectsQueryTicketBeforeRedemption(t *testing.T) {
	target := testTarget()
	redeemer := &fakeRedeemer{grants: map[string]TicketGrant{"subprotocol": grantFor(target, ScopeTerminalRead)}}
	server, pty := testServer(t, target, redeemer)
	httpServer := httptest.NewServer(server.http.Handler)
	defer httpServer.Close()
	defer pty.stop()
	offer, _ := muxproto.Offer("subprotocol")
	wsURL := "ws" + strings.TrimPrefix(httpServer.URL, "http") + muxproto.Path + "?ticket=query-secret"
	conn, response, err := websocket.Dial(context.Background(), wsURL, &websocket.DialOptions{Subprotocols: offer})
	if err == nil {
		_ = conn.CloseNow()
		t.Fatal("query ticket was accepted")
	}
	if response == nil || response.StatusCode != http.StatusBadRequest {
		t.Fatalf("query ticket status = %#v", response)
	}
	if redeemer.calls != 0 {
		t.Fatalf("query rejection reached redeemer %d times", redeemer.calls)
	}
}
