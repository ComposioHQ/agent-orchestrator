...
// Published routes shared with the Daytona launch contract.
const (
...
// ServerConfig contains only disposable-compute dependencies and identifiers.
type ServerConfig struct {
	...
	Mux          *PTYMux
	Observer     ports.WorkspaceObservation
	Logger       *slog.Logger
}

// Server is the authenticated published listener for one sandbox session.
type Server struct {
	cfg   ServerConfig
	http  *http.Server
	ready atomic.Bool
...
// NewServer validates and constructs a thin sandbox listener.
func NewServer(cfg ServerConfig) (*Server, error) {
	...
	if cfg.Observer == nil {
		cfg.Observer = &WorkspaceObserver{Root: cfg.WorkspaceDir}
	}
	if cfg.Logger == nil {
...
// Serve marks readiness and serves until Shutdown closes the listener.
...
// Shutdown stops the listener and terminates the direct PTY.
...
func (s *Server) handleMux(w http.ResponseWriter, r *http.Request) {
	...
	if r.URL.RawQuery != "" {
		http.Error(w, "query authentication is not accepted", http.StatusBadRequest)
		return
	}
	...
	defer remove()
	defer func() { _ = c.Close(websocket.StatusNormalClosure, "sandbox mux closed") }()
...
func (s *Server) handleObservation(w http.ResponseWriter, r *http.Request) {
	...
	observation, err := s.cfg.Observer.Snapshot(r.Context(), ports.WorkspaceInfo{Path: s.cfg.WorkspaceDir})
	if err != nil {
