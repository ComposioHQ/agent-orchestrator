...
const STAGES = ["preflight", "orchestrator", "delegation", "work-pr", "reviewer"];
...
function parseArgs(argv) {
  const o = { project: "", harness: "", orchestratorHarness: "", reviewerHarness: "", task: DEFAULT_TASK, stages: "all", orchestratorSession: "", workerSession: "", reviewerSession: "", mode: "tui", ao: "", report: "", pollTimeoutSeconds: 180, commandTimeoutSeconds: 120, pollIntervalSeconds: 3, tmuxLines: 160, allowUnobservable: false, cleanup: false };
  const flags = { "--project": "project", "--harness": "harness", "--orchestrator-harness": "orchestratorHarness", "--reviewer-harness": "reviewerHarness", "--task": "task", "--stages": "stages", "--orchestrator-session": "orchestratorSession", "--worker-session": "workerSession", "--reviewer-session": "reviewerSession", "--mode": "mode", "--ao": "ao", "--report": "report", "--poll-timeout-seconds": "pollTimeoutSeconds", "--command-timeout-seconds": "commandTimeoutSeconds", "--poll-interval-seconds": "pollIntervalSeconds", "--tmux-lines": "tmuxLines" };
